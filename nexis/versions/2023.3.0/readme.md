...
Revision History Date Release Changes 04/25/2023 2023.3.0 Added support for macOS 13 (Ventura) for client systems; see "Supported Revised the Aggregate Performance and Drive Rebuild numbers for F2 SSD systems (see "SSD Performance Updated values April 2023" on page 13 and "Drive Rebuilds and System Performance Updated SSD values April 2023" on page 14) Contents System Configuration Limits 10 System Performance (On-Premises) 12 Known Issues 14 Special Notes 20 Qualified and Approved Avid NEXIS Switches 20 Flow Control Required in Avid Production Networks 20 ATTO® ThunderLink™ 10 GbE and 40 GbE Adapters 20 VMware ESXi™ 20 VMware on Windows 20 Legal Notices 22 Accessing the Online Documentation The Avid NEXIS documentation is accessible in PDF format from the Avid NEXIS Knowledge Base.
...
Product Version Guidance Server and 2023.3.0 Recommended for all AvidAvid NEXIS systems and clients.
Clients See "What's New in Avid NEXIS Version 2023.3.0" on the next page.
Server and 2022.12.0 Minimum version for all Avid NEXIS systems and clients. Required for all Clients Avid NEXIS | EDGE workflows.
...
Avid NEXIS version 2021.8 is no longer recommended. Upgrade as soon as possible.
...
- The USM version is shown on the Controllers page and the Engines page in the Management Console,
as of Avid NEXIS version 2020.3.0.
...
USM BMC version Minimum Avid Supported Models Version NEXIS Version 1.37B 0.01.0041 2022.12.0 Avid NEXIS E-series, including Avid NEXIS | PRO 40TB with Controller 10 Avid NEXIS | SDA
> **Note:** Redundant Controllers in the same chassis must
both run USM 1.37B.
BON 45 2022.5.0 Avid NEXIS F-Series, Avid NEXIS | SDA+, and Avid NEXIS | PRO+ 1.37a 0.01.003c 2019.2.0 Avid NEXIS E-series, including Avid NEXIS | PRO 40TB with Controller 10
> **Note:** 1.37 0.01.003c 2019.2.0
Can use USM 1.37 and 1.37a in the same Engine for models that support dual controllers 4.1.16g 2.00.1093c 2019.2.0 Avid NEXIS | PRO 20TB and Avid NEXIS | PRO 40TB with purple fascia Controller 4.1.16d 2.0.0002 1.32 0.01.0034 7.9.1 Legacy Avid NEXIS E-series 1.16 0.01.0013 6.x Legacy Avid NEXIS E-series What's New in Avid NEXIS Version 2023.3.0 This is a major release that introduces the following new features and platforms:
...
numbers.
Within one Egnine, the power supply units must be the same type, either HE or ECO. In a shared- storage system of multiple Engines, there can be a mix of HE and ECO power supplies.
...
the upgrade, the Management Console contains a button to deactivate the current license. After the upgrade, that button is no longer displayed. After you upgrade to this version, your system will need a new license if you want to change the capacity or the renewal date. (See also "Avid NEXIS | Cloud Storage Issues" on page 15.)
...
Packs) 15.3TB drives, yielding a total Engine capacity of 153.6TB (one Media Pack) or 307TB (two Media
- Packs)
...
Current Avid NEXIS Version Can Upgrade Directly To On-premises systems: 6.x, 7.x, 2018.x, 2019.x, 2020.x, 2021.x, 2022.x 2023.3.0 Avid NEXIS | Cloud Storage systems: 2021.3.1 and newer 2023.3.0 To upgrade Avid NEXIS | Cloud Storage 2020.7.x and earlier to Avid NEXIS | FS version 2021.x, please contact Avid Customer Service or your sales representative to assist with the upgrade.
...
Windows 11 10 Build 21H2 See "Windows 10 Issues" on page 18.
10 Build 21H1 Operating Supported Versions Notes System 10 Build 20H2 (October 2020) 10 Build 2004 (May 2020) Windows Server 2022 with latest Service Pack 2019 with latest Service Pack 2016 with latest Service Pack 2012 R2 with latest Service Pack Supported Browsers in an Avid NEXIS On-Premises Environment The following are the minimum browser versions qualified for the current Avid NEXIS release stream. These browsers were qualified with the client operating systems supported in the current release.
...
Avid NEXIS system.
On-Premises Limits Item Embedded System Director System Director Appliance Media Packs (per 4, in any combination of Scale- 8 in a High Performance Storage Group system) Out and High Performance, 24 total (optionally including up to 8 in a High optionally including 1 Avid Performance Storage Group) with the Avid NEXIS | VFS NEXIS | Cloudspaces Media Standard or Avid NEXIS | FS Extended (Standard) license Pack and any combination of Avid NEXIS Engines 64 total (optionally including up to 8 in a High Performance Storage Group) with the Avid NEXIS | VFS Advanced or Avid NEXIS | FS Advanced license, and any combination of Avid NEXIS Engines Media Packs (per 1 in all 2U chassis (Avid NEXIS | E2, Avid NEXIS | E2 All-Mirror, Avid NEXIS | E2 SSD, Avid NEXIS | Engine) PRO, Avid NEXIS | F2, Avid NEXIS | F2X, Avid NEXIS | PRO+) 1-2 in Avid NEXIS | F2 SSD 1–2 in all 4U chassis (Avid NEXIS | E4, Avid NEXIS | E4 All-Mirror) 4–8 in all 5U chassis (Avid NEXIS | E5, Avid NEXIS | E5 NL, Avid NEXIS | E5 All-Mirror, Avid NEXIS | F5, Avid NEXIS | F5 NL) Storage Groups 4, optionally including one High 32, optionally including one High Performance Storage Performance Storage Group Group and one Avid NEXIS | Cloudspaces Storage and one Avid NEXIS | Group Cloudspaces Storage Group Workspaces 1024 (including Workspaces for 3072 (including Workspaces for Teams) Teams) Teams 19 80 Team Quotas Up to 255 Team Quotas per system (Teams have only one quota per Storage Group — which can be increased or decreased — but can have a quota in multiple Storage Groups) Team Admin No enforced limit Users Users (Clients) 5640 (including Users in Teams) Item Embedded System Director System Director Appliance User Groups 1028 (including User Groups in Teams) Group Non-Team users can belong to 71 non-Team user groups Memberships per Team users can belong to 71 user groups in the same Team User Connected 40 (Avid NEXIS E-Series/F- 165 with Avid NEXIS | VFS Standard or Avid NEXIS | FS (Active) Clients Series Engines) Extended (Standard) license 30 (Avid NEXIS | PRO or Avid 330 with Avid NEXIS | VFS Advanced or Avid NEXIS | FS NEXIS | PRO+) Advanced license Up to 10 to a High Performance Storage Group (Avid strongly recommends no more than 10) Files and Folders 8 million (Avid NEXIS E-series/F- 28.4 million Series configurations)
> **Note:** (Actual limit depends on file name length. Limit
3 million (Avid NEXIS | PRO/Avid assumes 67 ASCII characters (67 bytes). Longer NEXIS | PRO+ configurations) average file names reduce the file count limit.) Files per folder Avid recommends no more than 10,000 files per folder, particularly for Workspaces used by Avid MediaCentral | Production Management (formerly Avid Interplay | Production).
Pathnames Total bytes = 255 (Workspace name + path + filename).
Unicode characters can be multi-byte.
Cloud System Limits Item Limit Media Packs (per system) 1 (2PB capacity) Workspaces 3072 Teams 80 Workspaces, users, and groups created by a Team count against the overall system limits.
...
Item Limit characters (67 bytes). Longer average file names reduce the file count limit.) Files per folder Avid recommends no more than 10,000 files per folder, particularly for Workspaces used by Avid MediaCentral | Production Management (formerly Avid Interplay | Production).
Pathnames Total bytes = 255 (Workspace name + path + filename).
Unicode characters can be multi-byte.
...
Performance Type Rebuild Rate (Media Pack at 100% capacity) Drive Size Estimated Rebuild Time Scale-Out 48MB/sec 4TB 24 hours 0.17TB/hr or ~6hr/TB 6TB 36 hours 10TB 60 hours 14TB 84 hours 16TB 96 hours High 4TB 70MB/sec 16 hours 6TB 24 hours 0.25TB/hour, or ~4 hr/TB 10TB 40 hours 14TB 56 hours SSD 700MB/sec 3.48TB ~1.3 hours 2.52TB/hour, or ~24 min/TB 15.3TB ~6 hours
...
Avid NEXIS On-Premises Systems Issues Bug Number: SSENG-35018. If a system drive fails in an Avid NEXIS 5U system (Avid NEXIS | F5, Avid NEXIS | F5 NL, Avid NEXIS | E5, or Avid NEXIS | E5 NL) the system rebuilds the data on one of the system drives from the other one. The system might report that the rebuild is complete but the status message stops updating at 88% complete.
...
Bug Number: SSENG-31657. When configuring an Avid NEXIS F-Series Engine (including Avid NEXIS | PRO+ and Avid NEXIS | SDA+ using the Agent for initial setup, the enclosure ID does not remain set, and continues to display the factory default of 00.
...
Avid NEXIS | Cloud Storage Issues Bug Number: SSENG-35033. The Offline Activation method for updating a license does not work. ] Workaround: Use the Online Activation method to update a license on an Avid NEXIS | Cloud Storage system running a version prior to 2023.3.0.
Bug Number: (None). You might run into issues running clients with multiple NICs, each of which is able to connect to an Avid NEXIS | Cloud Storage system.
Workaround: In clients that are specifically connecting to Avid NEXIS | Cloud Storage, enable only one NIC.
Avid NEXIS Cloudspaces Issues Avid NEXIS | Cloudspaces was introduced in Avid NEXIS version 2019.4.
Bug Number: (None). If an Avid NEXIS system with Cloudspaces activated does not have NTP time synchronization enabled and its local time in UTC is behind (earlier than) the Azure time by 15 or more minutes, no communication to Azure can be made and all requests fail with the following authentication error: “Server failed to authenticate the request. Make sure the value of Authorization header is formed correctly including the signature.” Workaround: Configure NTP Server time synchronization on the Avid NEXIS system or manually adjust the local system time to less than 15 minutes different from the Azure time.
...
Bug Number: (None). If the Client Manager is connected to an Avid NEXIS system with dual controllers and the System Director service has switched over to the second controller, the Management Console launch button in the Client Manager does not redirect to the new System Director and cannot successfully open the Management Console.
...
3. Restart the Mac.
CentOS Issues Bug Number: SSENG-34797. The Avid NEXIS Client kit for CentOS does not install the Client Manager GUI.
...
Windows 10 Issues Bug Number: SSENG-29069. On Windows 10 clients, when using TeraCopy to move large numbers of files to an Avid NEXIS Workspace (instead of using Windows Explorer), TeraCopy sometimes treats a file as a directory, and the Windows client displays an error that it cannot move the file. Avid strongly recommends not using TeraCopy.
...
different workspace When one user maps a workspace to one drive letter, and another user maps a different drive letter
