...
Avid Pro Tools Software v2025.12 ReadMe for macOS This Read Me documents important compatibility information and known issues for all tiers of Pro Tools®
...
Workaround: The Pro Tools installation process may be interrupted by a dialog indicating, “The Pro Tools Helper must be closed before the software can be installed.” If you encounter this dialog, launch the macOS Pro Tools Helper View > Quit Activity Monitor (/Applications/Utilities/), select the process, choose Process, and then use the Force Quit option. Once the process quits, Pro Tools installation proceeds until completed.
...
In the Terminal, run “sudo mdutil -a -I off” to turn off Spotlight until the next reboot. Consider running this command when you know you will be using Pro Tools on startup.
...
Workaround: If you encounter this issue, right-click the command and choose Open.
...
PT-336324 Clicking outside an open modal dialog while Pro Tools is stopped results in any audio passing through the Pro Tools to cut out momentarily.
Workaround: Clicking outside an open modal dialog while Pro Tools is stopped results in a system alert chime and any audio passing through Pro Tools cuts out momentarily.
...
PT-256794 No signal on tracks in a session after switching sample rates in the Playback Engine.
...
Create Session from VENUE Workaround: If you encounter this issue, use again, but use a new (unique) name for the Session file.
...
Workaround: In certain cases with the HDX Playback Engine, when closing a session with a sample rate of session, you may not be able to play back the session. If this occurs, open the Playback Engine and change the Hardware Buffer Size to a different setting to resolve this issue.
...
Workaround: The only multichannel format Pro Tools supports for bouncing to MP3 is 5.1. Bouncing any Pro Tools becoming unresponsive and you will have to force quit.
...
Workaround: When using Avid Link in Pro Tools, as opposed to using the desktop client, incoming contact request notifications are visible in the Home screen under the Notifications tab. Go to the Avid Link Home, Notifications.
menu and choose then navigate to the Notifications section by clicking MP-5590 “Send Email Invitation” button not present in the Avid Link Find Talent window.
Send Email Invitation Workaround: The button in the Avid Link Find Talent window might not be visible when it is first opened. Refresh the page by clicking on a profile and then clicking the back button.
...
lower results in incorrectly mapped IO.
...
PT-329978 Drag and Drop to plugin sampler from a muted audio track results in silent audio (no sound).
...
Workaround: HD Native Thunderbolt is only supported as the first or second Thunderbolt device in a sample rate above 96 kHz. Visit the compatibility pages on the Avid website for more information (www.avid.com/compato).
...
Control Surfaces PT-248711 Performance issues may occur when using EuControl with the H/W Buffer Size set to 32 in large sessions on macOS.
...
Workaround: If blank space is included in the timeline selection, and the source video codec is Apple ProRes, Same as Source H.264, or PJPEG, then QuickTime export is not be permitted. If it is necessary to bounce Same as Source, either use DNx source material or replace any blank space with black frames before bouncing. Note that bouncing from ProRes to ProRes is possible by adjusting the QuickTime Settings in the QuickTime Bounce dialog.
...
video output may display squished during playback on the hardware display. To avoid this issue, try setting the engine Raster Size (in the I/O section of the video track) to an HD raster size (such as 1080x1920).
UME-1529 SD video will not stretch to a 16:9 aspect ratio on the desktop display.
Stretch setting enabled, the video window size updates to 16:9, but the picture does not stretch. You can workaround this by setting the engine Raster Size to an HD raster, such as 1920x1080. This can be done by Command-clicking (Mac) or Control-clicking (Windows) on the engine Raster Selector (in the I/O section of the video track).
through the HDMI or SDI connections.
standard. The HDMI and SDI outputs will not display these formats. Instead, DNxIP switches to 1080p in order to maintain the higher frame rate. Additionally, you must have installed the latest firmware update that is included with the driver. Visit www.avid.com for the latest driver compatibility information.
...
Workaround: If a software video pull factor of 0.1% is set while video hardware is disabled, enabling the video hardware may create an illegal pull setting. Video hardware only supports 0.1% pull ups at 24pPAL hardware is enabled, manually select an available option from the pull menu, which may be “none.” PTSW-178722 Video may not stay in sync when video hardware is attached and Pro Tools is locked to an external clock.
...
Pro Tools, or when the video peripheral is disabled. (THPV-32) Video Satellite 23.976 or 24 fps Sequence Start Time Ending In Digits Other than “:00” Causes Time Code Offset Workaround: Media Composer uses NTSC or PAL time code when setting the sequence start time regardless of the format selected by the user for display. When you are working on a video satellite project at 23.976 or 24 fps, a Media Composer sequence with a start time that ends in a value other than :00 causes an offset between Pro Tools and Media Composer at the session start. For example, the start time 45:02:10:15 workaround, always set your Media Composer sequence start time to a time code value ending in 00.
98460 Cueing from Media Composer in a 720p Project Causes 1-Frame Offset.
the project type—while Pro Tools counts at 25 or 29.97 frames per second. When you cue a sequence from the video satellite in a 720p project, the Pro Tools edit cursor can be offset by as much as one frame.
...
Dolby Atmos 7.1.2 bed's Lsr/Rsr channels when rendering.
beds can affect each other, with one of the settings overwriting the other. As a workaround, do not use 5.1 PT-315662 Dolby Audio Renderer loses signal when outputting to Core Audio Device after making input connections to Pro Tools Audio Bridge in Pro Tools.
...
Edit > Automation > Duplicate Dolby Atmos Plugin Automation to Pan
6. Select
Automation.
...
dragged and dropped to the timeline. To avoid this, use PT-253887 An audio driver error may be seen in the Dolby Atmos Renderer upon quitting Pro Tools.
...
Workaround: When real-time-bouncing Dolby Atmos ADM and/or Re-Render files, plugin delay is not accurately compensated if Renderer mode is External. To avoid this, bounce offline or enable the Internal Dolby Atmos Renderer. As of Pro Tools 2024.6, online bounce is compensated correctly in Internal Renderer mode.
AAX Immersive Audio Plugins PT-348828 Installing an immersive plugin while Pro Tools is running requires closing and reopening session to use the Renderer.
...
PT-351578 Newly added sends do not migrate when switching to a previously used renderer.
Workaround: When switching between renderers (such as Dolby Atmos to WalkMix 360RA), any send outputs added after initially changing the renderer do not automatically appear in the same slot when switched to the other renderer. If the same send path is desired for both renderers, manually re-assign the same path after switching renderers. Assignments are recalled per-renderer. Also note that automation persists with the send slot, not the assignment, so the same automation will apply to any assignment made to that slot, regardless of the current renderer.
PT-351236 If multiple tracks are assigned to the same Atmos Object, and that object path has a Master Fader, then on migration to another renderer, only one of the objects will be routed through the Master Fader.
Workaround: If you have several tracks assigned to the same Atmos Object controlled by a Master Fader track, only one random Object will be routed through the Master Fader after switching to a different renderer.
...
Workaround: When moving clips between a single multichannel track and multiple mono tracks (or the other way around), Pan Automation for 360RA WalkMix Renderer on those clips is lost. To avoid this issue, (Track > Split into Mono).
split the multichannel track into multiple mono tracks This is also workaround for migrating greater-than-stereo tracks assigned to objects in 360RA to Dolby Atmos.
...
Audio Vivid XU-95 If Pro Tools is set to a language other than English, Audio Vivid Renderer is in Chinese.
Workaround: The Audio Vivid renderer currently supports only English and Chinese. When Pro Tools is set to any language other than English in Pro Tools Preferences > Display > Language, the renderer defaults to Chinese.
XU-64 Import ADM Menu is not available if Pro Tools is set to any language other than English.
Workaround: The File > Import > To Audio Vivid option is only available when Pro Tools is set to English (Pro Tools > Preferences > Display > Language).
XU-30 Signal passes only when the Transport is running.
Workaround: With the Audio Vivid Renderer enabled, Pro Tools does not pass audio to the Master Fader or hardware outputs while the transport is idle. Audio signal flow to physical outputs is initiated only once the transport engages playback.
XU-81 Signal disappears on tracks assigned to Audio Vivid Renderer.
Workaround: Signal may disappear on tracks assigned to Audio Vivid Renderer after some time. If this happens, disable and re-enable the Audio Vivid Renderer.
XU-99 Enabling DSP mode, adding a hardware insert, or record-enabling tracks assigned to Audio Vidid Renderer causes signal loss in the renderer.
Workaround: When tracks are assigned to Audio Vivid Renderer, enabling DSP mode, adding a hardware insert, or record-enabling these tracks causes signal loss in the renderer making all assigned tracks silent.
Disable the Renderer first, then enable DSP mode, add a hardware insert, or record on a track, and then re- enable the renderer.
XU-43 Monitoring formats in Audio Vivid Renderer can exceed the width of the Pro Tools monitor path.
Workaround: It is recommended that you set the Audio Vivid Renderer Monitor setting to match the Monitor Path width and format configured in I/O Setup. Audio may not pan to the expected output channels if the Audio Vivid Renderer is set to a wider monitor path than that specified in I/O Setup.
XU-62 Audio Vivid Renderer Bounce window does not open when the Audio Vivid Renderer window is open.
Workaround: Close the Audio Vivid Renderer window before executing a Bounce Mix operation with Export Type is set to Audio Vivid Renderer.
XU-107 Audio Vivid Renderer latency varies across channels.
The first two channels of Audio Vivid Renderer that were assigned and enabled are not time-aligned with the others and are early by 1024 samples. This regularly occurs with the first channel, but less frequently with the second.
XU-116 Mirror setting in stereo Audio Vivid panner always reverts to Y Mirror when reopening a session.
Workaround: The Mirror setting in the stereo Audio Vivid panner always reverts to Y Mirror when reopening a session. This results in the panner displaying an inaccurate R channel location. Open the Audio Vivid panner and set it back to the expected mirror mode if you encounter this issue.
XU-110 Mirror setting in stereo Audio Vivid panner always reverts to Y Mirror when reopening a session.
Workaround: \When the Audio Vivid renderer is active, offline bounce of the physical monitor path may contain artifacts. For best results, use the Audio Vivid Export Type in the Bounce Mix dialog.
XU-109 When reopening a session, Pro Tools quits unexpectedly if both object output and path are inactive.
Workaround: When initially reopening a session where an Audio Vivid object output and the track to which it is assigned have both been deactivated, Pro Tools may quit unexpectedly. If you encounter this issue, relaunch Pro Tools and reopen the session, and the session should load normally.
XU-85 Automating one channel of a stereo Audio Vivid panner overwrites the automation of the other channel.
Workaround: When automating one channel of a stereo Audio Vivid panner in Free Drag mode, both channels write automation. The untouched channel writes a flat line, erasing any previously written data.
This prevents separate passes for each channel. To avoid this issue, complete automation for both channels in a single pass XU-75 Audio Vivid object ID may not be preserved in panner instances.
Workaround: When reopening a session, panner windows may display different Audio Vivid Object IDs than originally assigned. This can also occur when tracks are reordered and reactivated after being deactivated.
Be aware that object assignments may shift after session reopen or track reordering/reactivation.
XU-106 Audio Vivid is not delay compensated in exported files.
Workaround: When bouncing tracks assigned to the Audio Vivid renderer, the resulting audio signal is late compared to the source, depending on the delay applied to the tracks. You can manually edit the exported file after bounce to shift the offset audio earlier by the delayed amount.
XU-119 Imported Audio Vivid Bed tracks do not get bed output assignments during Audio Vivid ADM import.
Workaround: When importing an Audio Vivid ADM file, Bed tracks receive standard output assignments instead of being assigned to Audio Vivid beds. If you encounter this, add the bed assignments manually.
...
sample rates, please be aware that there is a known issue where may quit unexpectedly if adding the Control plugin following the instantiation of the Spatialiser plugin. To avoid this issue, use a different sample rate for your session.
...
PT-291453 On macOS Ventura, Pro Tools | Carbon devices may occasionally become de-acquired in Network Device Browser.
Workaround: On macOS "Ventura" the Network Device Browser in AMS may occasionally lose connection with Pro Tools | Carbon after rebooting the device. To reacquire the device it is recommended that you manually change the device sample rate AMS to any other than what is currently selected.
PT-282876 No ADAT output with Carbon Pre.
...
Routing Issues PT--307186 Missing track output or just R channel when flipping DSP mode on/off with all Headphones set to NOT mirror MON L-R. (PT-349383) Workaround: When using Pro Tools Carbon, if all headphones (HP1-4) are set not to mirror MON L-R in hardware setup, it may happen that the track output loses the R channel or both channels after disabling DSP mode on the last DSP-enabled track in session.
To avoid this issue:
- Enable DSP mode at least on one track in session.
- Set at least one Headphone to mirror MON L-R.
PT--307186 The Default Format cannot be changed to a format above stereo when Carbon stereo monitoring controls are enabled.
Workaround: The Default Format cannot be changed to a format wider than stereo in the Input, Output, Main (MON L-R) and Insert tabs of the I/O Setup when the monitor is enabled in the Monitors section in the Main tab of the Hardware Setup. Disable the Main (MON L-R) monitor in the Hardware Setup to be able to set the Default Format to a format wider than stereo.
...
MON L–R.
To correct this, assign the Audition Path to any output except After applying this workaround, the Audition Path remains active while the first track in the session will be inactivated due to insufficient resources.
...
Playback and Recording Issues PT-282647 Frequent –6101 errors with Carbon Expanded AVB streaming at 192 kHz an with a low HW Buffer Size.
...
some downstream tracks. To avoid this issue, route the signal through separate buses rather than through sub-paths of the same multichannel bus.
...
Pro Tools or Carbon Central Becomes Unresponsive or Quits Unexpectedly PT-266976 After quitting unexpectedly, Pro Tools cannot communicate with Pro Tools | Carbon.
...
PT-266576 On systems that support sample rates higher than 96 kHz, the system may become temporarily or 192 kHz and vice versa.
...
Error –6042 Workaround: If you repeatedly encounter –6042 errors, open the System Usage window and check the PCI gauge. If the gauge is peaked, you will need to reset the PCI bus. To reset the PCI bus, make all tracks inactive and start playback. If you still encounter a –6042 error, quit and relaunch Pro Tools, open the Number of Voices Playback Engine dialog, and change the setting to use fewer voices per DSP.
...
Errata What's New in Pro Tools 2025.12 Guide and Pro Tools Reference Guide (2025.12) Import to Audio Vivid The What's New in Pro Tools 2025.12 Guide.pdf and the Pro Tools Reference Guide (2025.12) omit the procedure for importing an Audio Vivid ADM file (macOS only).
To import an Audio Vivid ADM file into a session:
1. Create and configure a new session for the Audio Vivid ADM file (or open an existing session).
2. Ensure that the Audio Vivid Renderer is enabled.
3. Choose File > Import > Import to Audio Vivid.
4. In the resulting Import dialog, click Select and navigate to and select the Audio Vivid ADM file that
you want to import, then click Open.
5. Click Confirm.
Compensate HW Inputs The What's New in Pro Tools 2025.12 Guide.pdf and the Pro Tools Reference Guide (2025.12) omit the new Compensate HW Inputs (Setup > Preferences > Mixing).
option Enable the Compensate HW Inputs option to automatically compensate for latency with hardware inputs.