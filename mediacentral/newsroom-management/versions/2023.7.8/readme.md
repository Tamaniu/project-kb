...
Avid MediaCentral | Newsroom Management Avid MediaCentral | Newsroom Management Version 2023.7.8 ReadMe Avid recommends that you read all of the information in this ReadMe thoroughly before installing or using the software described in this document.
...
https://kb.avid.com/articles/en_US/user_guide/MediaCentral-Newsroom-Management Revision History Date Revised Changes Made 2 Feb 2026 Added upgrade information and fix related to 2023.7.8 22 August 2025 Added upgrade information and fix related to 2023.7.7 3 March 2025 Added fixes related to 2023.7.6 5 December 2024 Added upgrade info and new fixes related to 2023.7.5 11 June 2024 Updated Limitations section for Versions 2023.7.x 30 May 2024 Updated Security Guidelines and added multiple updates, including 7 fixes related to 2023.7.4 14 February 2024 Added 3 fixes related to 2023.7.3 22 January 2024 Added 2 fixes related to 2023.7.2 19 December 2023 Updated to add information related to 2023.7.2 16 October 2023 Updated to add information related to 2023.7.1 2 August 2023 Updated to remove previously documented sections related to releases prior to 2023.7 27 July 2023 Initial publication: Updated compatibility content, and added some new features and fixed issues for 2023.7 Avid MediaCentral | Newsroom Management Contents Downgrade Notes and Issues 10 New Features in MediaCentral | Newsroom Management 2023.7 11 Fixed in Version 2023.7.8 12 Fixed in Version 2023.7.7 12 Fixed in Version 2023.7.6 12 Fixed in Version 2023.7.5 13 Fixed in Version 2023.7.4 13 Fixed in Version 2023.7.3 14 Fixed in Version 2023.7.2 14 Fixed in Version 2023.7.1 14 Fixed in Version 2023.7 15 Limitations and Defects found in Version 2023.7.x 15 Limitations and Defects found in Version 2023.3 16 Limitations and Defects for Earlier Versions 16 Configuring Gmail SMTP Relay Server for Sendmail 19 Updating the iNEWS CTC Service 20 Doc Errata 22 Technical Support Information 22 Security Guidelines Security Guidelines Avid allows you to install an endpoint detection and response solution on your Newsroom Management system. This applies to both single-server and cluster environments. While Avid does not support any specific solution, you can find general guidelines and information related to CrowdStrike Falcon on the following Avid Knowledge Base page: https://kb.avid.com/articles/en_US/troubleshooting/en239659 Compatibility Notes and Issues This section contains compatibility information. Specific versions are mentioned when pertinent.
...
MCNM 2022.3.x 2022.12 2023.3.x 2023.7.x A1 2022.3.x A A A NA1 2022.12 A A A 2023.3.x NA NA A A 2023.7.x NA NA NA A 1A = Allowed | NA = Not Allowed
...
Compatibility Notes and Issues Servers and Operating Systems Compatibility Matrix The following matrix displays compatibility information for Newsroom Management version 2022.3 and later.
...
Upgrading to 2023.7.8 This is a client only release. Before upgrading to v2023.7.8 from any version prior to 2023.7.4, you must first complete the installer procedure for upgrading to v2023.7.4 and 2023.7.5, as described in this Readme. Then, do the following procedure to complete the upgrade process:
...
3. In that story, insert the client version, 9.1.8.7 , onto a separate line.
4. Then, on each Newsroom Management Server, run the following commands:
offline configure -n online Upgrading to 2023.7.7 This is a client only release. Before upgrading to v2023.7.7 from any version prior to 2023.7.4, you must first complete the installer procedure for upgrading to v2023.7.4 and 2023.7.5, as described in this Readme. Then, do the following procedure to complete the upgrade process:
1. At an iNEWS Workstation, open the queue SYSTEM.CLIENT.VERSIONS.
2. Open the first story (usually named “versions”) in that queue.
3. In that story, insert the client version, 9.1.7.7 , onto a separate line.
4. Then, on each Newsroom Management Server, run the following commands:
offline configure -n online Upgrading to 2023.7.6 This is a client only release. Before upgrading to v2023.7.6 from any version prior to 2023.7.4, you must first complete the installer procedure for upgrading to v2023.7.4 and 2023.7.5, as described in this Readme. Then, do the following procedure to complete the upgrade process:
1. At an iNEWS Workstation, open the queue SYSTEM.CLIENT.VERSIONS.
2. Open the first story (usually named “versions”) in that queue.
3. In that story, insert the client version, 9.1.6.8 , onto a separate line.
4. Then, on each Newsroom Management Server, run the following commands:
offline configure -n online Install and Upgrade Notes and Issues Upgrading to 2023.7.5 Before upgrading to v2023.7.5 from any version prior to 2023.7.4, you must first complete the installer procedure for upgrading to v2023.7.4, as described in this Readme. Then, do both of the following procedures (related to dbrestore and client versioning) to complete the upgrade process:
To update dbrestore:
1. Copy the dbrestore.gz to the tmp folder on each Newsroom Management Server and unzip with
“gzip -d dbrestore.gz”.
2. On each Newsroom Management Server, run the following commands to back up the old dbrestore
executable and put the new one in place:
cd /exc mv dbrestore dbrestore.orig mv /tmp/dbrestore /exc/dbrestore sudo chown root:newssys dbrestore sudo chmod 4755 dbrestore To add the client version to the database:
1. At an iNEWS Workstation, open the queue SYSTEM.CLIENT.VERSIONS.
2. Open the first story (usually named “versions”) in that queue.
3. In that story, insert the client version, 9.1.5.5, onto a separate line.
4. Then, on each Newsroom Management Server, run the following commands:
offline configure -n online Upgrading to 2023.7.4 Upgrading to 2023.7.4 from previous version of 2023.7 or 2023.3 is a standard upgrade without database migration required.
> **Note:** Upgrading from versions prior to 2023.3 is not supported.
If internet access is available, before running the command, ./install upgrade, you need to uncomment (remove the # before) the following line for the repository in /etc/apt/sources.list:
...
Install and Upgrade Notes and Issues If internet access is unavailable, prepare additional OS files for Offline Installation:
1. Ensure your Ubuntu environment (desktop or server installation workstation) has internet access.
download-dependencies.sh,
2. Copy the script, from the Linux folder in the iNEWS ISO to your
...
Install and Upgrade Notes and Issues Upgrading to 2023.3 Upgrading to 2023.3 from previous version(s) is not supported.
...
Exporting the database Exporting the current database is required for going to version 2023.3.
...
2023.7.x Downgrading to previous version of 2023.7 or 2023.3 is a standard downgrade.
...
Fixed in Version 2023.7.8 Feature Description Topline messaging Users at sites employing more generic user IDs can now see real user names in topline supports User Real messaging at the iNEWS Workstation, making for a more user friendly experience.
...
Fixed in Version 2023.7.8 The following issues have been resolved in MediaCentral | Newsroom Management v2023.7.8:
Bug Number: IN-180631, IN-180818, IN-180870. Previously when a user selected or inserted production cues or when the user scrolled in a story with production cues, the user could experience the cursor position issues. The view would sometimes jump to the beginning of the story, or the story view would scroll to the last cursor position. Additionally, queue navigation might've been impacted and the cursor automatically returned to the story panel.
Fixed in Version 2023.7.7 The following issues have been resolved in MediaCentral | Newsroom Management v2023.7.7:
Bug Number: IN-180769. In a previous version, macros using the right Control (Ctrl) key failed to execute as expected.
Fixed in Version 2023.7.6 The following issues have been resolved in MediaCentral | Newsroom Management v2023.7.6:
Fixed in Version 2023.7.5 Bug Number: IN-180528. Previously, iNEWS Workstations did not correctly return Unicode characters, specifically those with macron accents, to HTML5 plugin.
...
Fixed in Version 2023.7.5 The following issues have been resolved in MediaCentral | Newsroom Management v2023.7.5:
...
Bug Number: IN-180505. In previous versions, macros would stop working after a Windows 11 update.
Fixed in Version 2023.7.4
> **Note:** Along with Newsroom Management v2023.7.4, a newer version of Data Receiver was also released;
please see the Data Receiver v2023.7.4 ReadMe for more information.
The following issues have been resolved in MediaCentral | Newsroom Management v2023.7.4:
...
Fixed in Version 2023.7.3 Bug Number: IN- 180072 In previous versions, if users signed onto the iNEWS Workstation without "-a" or "- b" and launched the HTML5 plugin, they will get an error of "Failed to load URL - http://.....".
...
Bug Number: IN- 179719. In previous versions, if there is a line with only a space in the HTML5 configuration, it could cause the parser to become stuck.
...
Limitations and Defects found in Version 2023.7.x The following limitations or defects were discovered in this release of Newsroom Management.
Bug Number: IN- 180261. ∙For 2023.7.x versions, external users could not be used for mgwcom connections, which are used by MOS Gateway Workarounds Limitations and Defects found in Version 2023.3 Either use normal/internal users, or you could make external user work by running the following command sudo cp /etc/pam.d/rxnet /etc/pam.d/mgwcom at the server:
At Newsroom Management sites with subscription licensing, users at iNEWS Workstations may experience a short delay at log in while the Newsroom Management system transitions to grace period mode, because the MediaCentral Server goes offline.
...
Limitations and Defects for Earlier Versions Divehi Support With the new richedit control, sometimes the last line of text will align to the left for a right-aligned story.
...
Limitations and Defects for Earlier Versions Plug-ins Some plug-ins may not paint correctly in Newsroom Management.
...
printing.
Configuring Gmail SMTP Relay Server for Sendmail There are screen painting issues when the Vertigo Xmedia plug-in and the Aurora Assignment List
...
MCNM-A# ls -l /exc/inewsctc -rwxrwxr-x. 1 so newssys 4559040 July 6 2023 inewsctc MCNM-A# version /exc/inewsctc inewsctc: 2023.7.1 (build 9.1.1.2) RH7 If this shows anything earlier than the version prior to the current version to which you are updating, abandon the upgrade and start the devices: start 601 602 603 in this example.
...
MCNM-A# chown so:newssys /exc/inewsctc MCNM-A# chmod +x /exc/inewsctc Updating the iNEWS CTC Service
7. Check the properties of the new inewsctc executable, and start the devices. For example:
MCNM-A# ls -l /exc/inewsctc -rwxrwxr-x. 1 so newssys 4579704 Dec 19 03:18 inewsctc MCNM-A# start 601 602 603 Doc Errata Doc Errata MediaCentral Newsroom Management Setup and Configuration Guide In Appendix A of the 2023.3 version of the guide, in the Job List Commands section, the command bpoll has the following note:
