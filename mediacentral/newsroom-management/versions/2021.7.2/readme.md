...
source-file: MCNM_v2021.7.2_ReadMe.pdf
confidentiality: public
date-added: 28/04/2026
status: superseded
superseded-by: "2021.11"
feature-tags: [release-notes, patch-release, workserver, bug-fixes, upgrade, downgrade, systemd, gmail-smtp, known-issues, rhel, centos, unc-paths, multiple-monitors, macros, web-services-api, data-receiver, mos-gateway]
---

# MediaCentral | Newsroom Management — Version 2021.7.2 ReadMe
...
> **Source document date note:** The v2021.7.2 ReadMe does not contain a separate copyright page. The document date (12 December 2022) is taken from the revision history entry in the source PDF. This is the best available confirmed date for the `release-date` field above.
...
| Date | Notes |
|---|---|
| 12/12/2022 | Initial v2021.7.2 release notes |
...
Newsroom Management v2021.7.2 is compatible **only** with other v2021.7.x components. Cross-version operation is not supported.

| Component | Compatible Version |
|---|---|
| Newsroom Management Server | v2021.7.2 |
...
The following features were introduced in prior versions and remain active in v2021.7.2. See the "Changes between" sections below for full detail.
...
### Upgrading to v2021.7.2

> **Important: v2021.7.2 is a workserver binary swap only — it is NOT a full installer upgrade.**

The v2021.7.2 update is distributed as `iNEWS_2021.7.2.zip`, which contains only the updated `workserver` executable. Do not attempt a standard full installer upgrade with this package.

**Procedure:**

1. Stop the iNEWS services:
   ```bash
   systemctl stop inews-*
   ```
2. Back up the existing workserver binary:
   ```bash
   cp /exc/workserver /exc/workserver_old
   ```
3. Extract the new `workserver` binary from `iNEWS_2021.7.2.zip`.
4. Copy the new binary to the iNEWS executables directory:
   ```bash
   cp workserver /exc/workserver
   ```
5. Set the correct ownership:
   ```bash
   chown root:newssys /exc/workserver
   ```
6. Set executable permissions:
   ```bash
   chmod +x /exc/workserver
   ```
7. Restart iNEWS services:
   ```bash
   systemctl start inews-*
   ```

### Downgrading from v2021.7.2

To roll back to v2021.7.1, stop iNEWS, restore the backed-up binary, and restart:

```bash
systemctl stop inews-*
cp /exc/workserver_old /exc/workserver
chown root:newssys /exc/workserver
chmod +x /exc/workserver
systemctl start inews-*
```

> **Note:** This rollback procedure is only valid if the `workserver_old` backup from the upgrade procedure above is still present.

### Downgrading from v2021.7.1 or Earlier

See the v2021.7.1 ReadMe for downgrade procedures applicable to those versions. Downgrading from v2021.7 or v2021.7.1 requires the old database file; contact Avid Customer Support before attempting.
...
## Changes between 2021.7.1 and 2021.7.2

One bug fix:

| ID | Summary |
|---|---|
| IN-179393 | Workserver miscalculated the session count after a service restart, causing sessions to be incorrectly reported as in use. Fixed. |
...
| ID | Summary |
|---|---|
| IN-7112 | Null-padded display names caused invalid NSML on `dbrestore`. Fixed. |
