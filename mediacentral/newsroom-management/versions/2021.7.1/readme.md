...
source-file: MCNM_v2021.7.1_ReadMe_RevB.pdf
confidentiality: public
date-added: 28/04/2026
status: superseded
superseded-by: "2021.7.2"
feature-tags: [release-notes, patch-release, isam, real-name-display, bug-fixes, upgrade, downgrade, systemd, grpcheck, ssh, gmail-smtp, known-issues, rhel, centos, unc-paths, multiple-monitors, macros, web-services-api, data-receiver, mos-gateway]
---

# MediaCentral | Newsroom Management — Version 2021.7.1 ReadMe

**Build:** v7.9.1  
**Document date:** 8 February 2022 (Rev B)  
...
> **Note:** Since this release of MediaCentral Newsroom Management will be more widely distributed than previous versions, some notes on important features and changes have been brought forward from previous ReadMe documents.

> **Source document date note:** The document body states a revision date of "19 January 2021" for the v2021.7.1 release. This is internally inconsistent: v2021.7 itself was dated 26 July 2021, so v2021.7.1 cannot pre-date it. The copyright page of RevB confirms "8 February 2022" and Copyright © 2022. The "19 January 2021" date is almost certainly a typographical error in the source document (likely "19 January 2022"). The `release-date` field above uses the RevB document date (08/02/2022) as the best available confirmed date.
...
| Date | Notes |
|---|---|
| ~19/01/2022 | Rev A — initial v2021.7.1 release notes (body date; likely "19 January 2022" — see source note above) |
| 08/02/2022 | Rev B — expanded document; copyright page confirms this date |
...
Newsroom Management v2021.7.1 is compatible **only** with other v2021.7.x components. Cross-version operation is not supported.

| Component | Compatible Version |
|---|---|
| Newsroom Management Server | v2021.7.1 |
| iNEWS Workstation | v2021.7.x |
| MOS Gateway | 2021.3 or later |
| Data Receiver | v1.8 or later |

> **Note:** A v2021.7.x client cannot connect to a server running a version older than v2021.7.x. Older clients can connect to a v2021.7.x server, subject to feature limitations.
...
> FTS/DR Server and MOS Gateway are qualified on Windows Server 2016 and Windows Server 2019. iNEWS Workstation is qualified on Windows 10 Pro 64-bit.

---

## Features Carried Forward from Previous Versions

The following features were introduced in prior versions and remain active in v2021.7.1. See the "Changes between" sections below for full detail.

| Feature | Introduced |
|---|---|
| Real Name Display (`display_user_realname=YES`) | v2021.7 |
| iNEWS field tag format change (`<f>` → `<string>`) | v2021.7 |
| NSML v4.2 | v2021.7 |
...
### Upgrading to v2021.7.1

> **Critical: ISAM export/import is required when upgrading from v2021.7 to v2021.7.1.**

Fix IN-7112 (null-padded display names causing invalid NSML) requires that ISAM data be re-imported after the upgrade so that display names are validated and cleaned. Without this procedure, corrupt display name data from v2021.7 may persist.

**Procedure:**

1. Before upgrading, export the ISAM database:
   ```
   dbdump Ci -f /tmp/isam_dump
   ```
2. Perform the v2021.7.1 upgrade.
3. After the upgrade, re-import the ISAM data:
   ```
   dbgen u
   dbrestore i -f /tmp/isam_dump
   ```

**Real Name Display — post-restore reindex requirement:**

If **Real Name Display** (`display_user_realname=YES`) is enabled **and** FTS/mediaindex are in use, a full reindex is required after the `dbrestore i` step above.

### Downgrading from v2021.7.1

Downgrading from v2021.7.1 requires the **old database file** (not just the ISAM export) from a backup taken prior to the upgrade. Contact Avid Customer Support before attempting a downgrade.

### Downgrading from v2021.3

Requires the old **ISAM** export.

### Downgrading from v2020.9.x and Earlier

Requires manual uninstall of the software plus deletion of:

```
/etc/systemd/system/iNEWS.service
```
...
Service logs are recorded per service and can be found in `/var/log/iNEWS/`. The bioserver service file is located at `/usr/lib/systemd/system/inews-bioserver.service`.
...
## Changes between 2021.7 and 2021.7.1

Four bug fixes:

| ID | Summary |
|---|---|
| IN-7112 | Null-padded display names caused invalid NSML on `dbrestore`. Fixed. ISAM export/import procedure (see Upgrade Notes) is required when upgrading from v2021.7. |
| IN-7215 | `dbrestore tdv` broke the internal UUID table. Fixed. |
| IN-7180 | Modify-by field not displayed after data conversion. Fixed. |
| IN-7304 | Carriage return in Realname field caused client crash on story save. Fixed. |
...
- In previous versions, a Connect to Service dialogue would go to a Login screen instead of prompting for password. This is fixed. IN-3503
