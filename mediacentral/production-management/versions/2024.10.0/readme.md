...
Avid MediaCentral | Production Management Version 2024.10 ReadMe Avid recommends that you read all of the information in this ReadMe thoroughly before installing or using the software described in this document.
...
https://kb.avid.com/articles/en_US/readme/MediaCentral-Production-Management-v2024-x- Documentation MediaCentral Production Management v2024.10 is distributed as a Long-Term Maintenance release. For more information on the differences between Long-Term Maintenance and Feature releases, visit the Avid website at: https://www.avid.com/resource-center/avid-long-term-maintenance-releases Revision History Date Revised Changes Made June 26, 2025 Updated statement regarding "Support for Virtual Environments" on page 4.
Added IPI-3853 to Limitations for Production Management 2024.10.
April 10, 2025 Added Limitations for Production Management 2024.10.
Added IPI-3732 to "Limitations for Production Management 2023.7.x" on page 29.
February 21, 2025 Added two limitations to "Limitations for Production Management 2023.7.x" on page 29.
One regarding the use of OpenID authentication providers, and the other relates to an issue with the system recognizing the correct CSS value after a system reconfiguration.
Clarified the requirements for "Manually Uninstall Before Upgrading" on page 7.
November 20, 2024 Formally listed as a Production Management limitation for MediaCentral Sync, ICI-4932 has been moved to the MediaCentral Cloud UX ReadMe as that is where the fix will be implemented.
November 14, 2024 Initial v2024.10 publication.
...
Supported Operating Systems and Hardware Simplified Chinese and Traditional Chinese
- Japanese
...
Multi-byte languages (Simplified Chinese, Traditional Chinese, and Japanese) are not supported by Production Services.
SGL FlashNet includes support for Simplified Chinese, which is qualified for use with Production Management Archive and Restore services. See also the topic “International Character Support Limitations” in the MediaCentral Production Management v2021.3 ReadMe.
...
For more information on supported operating systems and Avid software interoperability, see the following article on the Avid Knowledge Base: https://kb.avid.com/articles/en_US/compatibility/Avid- Video-Compatibility-Charts For details on Production Management server hardware requirements, see the MediaCentral | Production Management Hardware Guide.
...
Component Windows DEP User Account Control Settings .NET Framework v3.5 Transcode Required Required Not Required Production Services Not Required Not Required Required Engine STP Encode Required Required Required Transfer Not Required Not Required Required (Windows 10 / 11) Enable the “Turn on DEP for essential Windows programs and services only” option in Windows System Properties.
2 Change User Account Control settings to "Never notify".
Support for HPE MSA 2060 on Production Management Cluster Systems The HPE MSA 2060 has been qualified for use with Production Management v2024.10.
...
Security Guidelines Support for Virtual Environments As of June 2025, Avid is no longer testing or qualifying the deployment of MediaCentral Production Management in a virtual environment using any specific vendor (such as VMware). While Avid allows you to deploy Production Management in a virtual environment, the selection and qualification of the virtualization solution is the responsibility of an individual or team within your own organization.
If you plan to run your systems on a virtual environment, please keep in mind the following requirements and best practices:
- Unless otherwise noted, Avid requires that you allocate the same type and amount of system
resources to a virtualized server as you would for a dedicated, hardware-based (“bare-metal”) server. Resource allocation must be aligned with your organization’s requirements and use cases.
For more information on system requirements, see the MediaCentral | Production Management Hardware Guide.
- Avoid overloading your host resources to ensure proper system performance.
- Verify the compatibility of the applications to be installed on the VM (applies to both Avid and other
3rd-party applications).
Apply security patches to your virtualization solution on a quarterly basis (at minimum). If higher
- security risks are identified, shorter intervals of time are recommended.
Consult your virtualization vendor’s documentation for more information about configuration, as
- well as recommendations and best practices.
Security Guidelines Avid allows you to install an endpoint detection and response solution on your MediaCentral Production Management servers. This applies to both single-server and cluster environments. While Avid does not support any specific solution, you can find general guidelines and information related to CrowdStrike Falcon on the following Avid Knowledge Base page:
...
Apache Log4j Updates This release includes updates for components previously running Apache Log4j v1.x or v2.17.0 to v2.17.1.
> **Note:** The Production Management Engine continues to use one Apache Log4j 1.x component.
For more information, see Avid's Apache Log4j Security Assessment documentation on the Avid Knowledge Base at: https://kb.avid.com/articles/en_US/troubleshooting/en239659 New for Production Management v2024.10 New for Production Management v2024.10 This section contains information for all of the new features and improvements for the Production Management v2024.10 release.
End of Support Announcement for macOS 12.x (Monterey) Production Management v2024.10 is the last release to support macOS 12.x (Monterey). Any organizations that are still using this version of macOS should begin to think about an OS update. In some cases, this update could have an impact to hardware as not all Mac hardware supports the latest versions of macOS.
For more information on supported operating systems and Avid software interoperability, see the following article on the Avid Knowledge Base: https://kb.avid.com/articles/en_US/compatibility/Avid- Video-Compatibility-Charts Interplay Administrator The MediaCentral Search Connector section of the tool has been updated to include user interface changes that vary, depending on your version of MediaCentral Cloud UX.
For additional information about these options and the Asset Visibility feature referenced below, see "Configuring the MediaCentral Search Connector" in the Avid MediaCentral | Cloud UX Installation Guide.
When Connecting to MediaCentral Cloud UX v2023.7 v2023.7.0 of the Configuration section of the tool included a check box that allowed you to Relax Asset Visibility for nested assignments. v2023.7.1 replaced this button with a menu that allowed for two different options: Relax or Restrict. v2024.10 adds a new Retain option to this menu.
...
New for Production Management v2024.10 When Connecting to MediaCentral Cloud UX v2024.10 If you connect Interplay Administrator v2024.10 to a MediaCentral Cloud UX v2024.10 system, the user interface updates (per the following example illustration).
Note the following changes:
- The Assign section of the tool is removed. The avidctl platform config pam script is used to
define this information.
The Configuration section allows you to enable or diable Asset Visibility. If you select the Enable
- User Permissions check box, the system functions as if the Retain option (described above) is
enabled.
- Formally available in the Property selection tab, the Update Data Type Definitions check box has
been relocated to this screen.
- The Clean-up setting allows you to reset the search connector settings back to its original state
(when you completed the initial Resync). This button might be useful in the event that a setting is accidentally changed or misconfigured on the MediaCentral Cloud UX system postdeployment.
The Status area of the tool (left) now includes Production Server and Connected User values. The
- avidctl platform config pam
Connecter User is the user that you define when running the script on the MediaCentral Cloud UX system.
You must use v2024.10 of the Interplay Administrator to configure the v2024.10 MediaCentral Cloud UX Search Connector. This is true even if you are connecting MediaCentral Cloud UX v2024.10 to a 2023.7 Production Management system.
MediaCentral Delivery Compatibilty The MediaCentral Production v2023.7.3 update changed the default password used by Delivery. If you need to use Delivery to exchange assets between Production Management v2024.10 and v2023.7.x, you must upgrade your v2023.7 Delivery Receiver to v 2023.7.3 or later.
...
Before upgrading any Production Management software on a server or a client workstation, note the following requirements:
Do not manually uninstall the Production Management Engine or Archive Engine software. The
- MediaCentral Production Management installer program uninstalls the previous version
automatically as part of the installation process.
- Manually uninstall the Production Services Engine software. However when prompted, do not
uninstall the SQL server or SQL database.
The SQL database is used as a repository for the Production Services profiles, and other settings.
> **Note:** Depending on the version of software, you might know the Production Services Engine under
its former names: Media Services Engine, or DMS Broker.
If you have installed v1.8.1 of the Avid Service Framework on any system, you are not required to
- uninstall / reinstall this software as the version is not subject to change in this release.
- Manually uninstall all other Production Management components (applies to all servers and client
workstations).
...
Knowledge Base.
To uninstall, and reinstall the Production Management software:
...
For the recommended upgrade procedure, see "Updating the Avid Production Management Engine Software" on page 14.
...
C:\Program Files\Avid\Avid Interplay Engine\Logs\Machines\<machine_name> Upgrading MediaCentral Production Management Examine and Archive the Logs After the Upgrade If you use the Collect tool, examine the Base Analyzer portion after the upgrade. This portion should run successfully without any errors. Archive the output of the tool as a record of the state of the system after the upgrade.
Database Changes for Upgrading to v2019.x Upgrading from any release earlier than Interplay v3.2 requires a database upgrade. Make sure you back up your database before the upgrade and lock the database when you perform the upgrade as described in "Updating the Avid Production Management Engine Software" on page 14.
...
See "Best Practices for Performing an Engine Upgrade" on page 7.
...
Upgrading MediaCentral Production Management Because the process of removing the links can take a long time, it is important to follow the upgrade procedure described in "Upgrading MediaCentral Production Management" on page 7. If you follow the procedure, the database change occurs while you are logged into the Interplay Administrator. During the process, the Interplay Administrator displays a progress bar. Messages are also displayed in the Production Management Engine log file, nxnserverlog. You can periodically open the log file with Notepad to monitor the progress.
During the database upgrade, the Production Management Engine also determines if there are rendered effects that are not currently in use and moves these rendered effects to one or more subfolders in the Orphan Clips folder. An administrator or media manager can then delete these assets and their media.
For more information about options for deleting, see "Deleting Unused Rendered Effects After an Upgrade" on page 18.
...
> **Note:** Depending on the size of your database, this kind of backup can take a long time. To save time
you can temporarily move the _Backups folder from within the AvidWG (or AvidAM) folder. to another location. After you perform the copy you can move the folder back under AvidWG.
...
Upgrading MediaCentral Production Management Updating the Avid Production Management Engine Software The following topics describe how to update the Production Management Engine and Archive Engine software:
- "Upgrading a Production Management Engine Cluster or Archive Engine Cluster" below
- "Upgrading a Non-Clustered Production or Archive Engine" on the next page
Upgrading a Production Management Engine Cluster or Archive Engine Cluster The process described here is similar to a rolling update as documented in the MediaCentral Production Management Engine Failover Guide, except that the database is locked and deactivated before you begin.
...
Engine Database" on page 10.
2. If you took the Avid Workgroup Engine Monitor resource offline when you performed a backup,
bring all of the resources inside the cluster resource group online. Right-click the “Avid Workgroup Server” group and select Start Role. For more information, see the final steps in "Backing Up the Production Management Engine Database" on page 10.
...
> **Note:** You can also browse to the appropriate .pro file and then click Activate. On a cluster you
must browse from the network, not from the mount.
...
The first time you open the Production Management database after an upgrade from a version earlier than v2.3, the Production Management Engine automatically removes links for rendered effects. In a large database, this can take several hours (see "Activating the Database" on page 16). During this process, the engine also determines if there are rendered effects that are not currently in use and moves these rendered effects to one or more subfolders in the Orphan Clips folder. An administrator or media manager can then delete these assets and their media.
...
C:\ProgramData\Avid\GenericFTP\FtpProflies.xml Upgrading Media Indexer Servers When upgrading a Media Indexer server, you must do the following:
...
Upgrading Production Services Engine
...
Upgrading Production Services Engine When upgrading a Production Services Engine server to v2024.10, please note that some changes are required to support a future upgrade path with MSSQL 2019, and the following issues have been identified:
...
Upgrading Production Services Engine Uninstalling SQL 2012 and Installing Production Services Engine This section shows you how to uninstall SQL 2012 and install Production Services Engine v2022.3. This was done on Windows 2019 Server.
...
For more information, see the following topics:
  ...
  - "Importing Your Backed Up Production Services Engine Profiles" on page 26.
Upgrading Production Services Engine Checking the Production Services Engine Configuration Settings After successfully installing Production Services Engine and SQL Express 2019, the Settings were retained in this process To check the Production Services Engine Configuration Settings:
...
Upgrading Production Services Engine Importing Your Backed Up Production Services Engine Profiles After successfully installing Production Services Engine and SQL Express 2019, the Settings were retained in this process To import your backed up Production Services Engine Profiles:
...
Fixed in Production Management v2024.10 Fixed in Production Management v2024.10 The following issues have been resolved in this release of MediaCentral Production Management.
Installation and Configuration Bug Number: RELENG-5136. Some components of the Production Management installation package might be flagged by Windows as coming from an Unknown Publisher. This is likely to occur if you have Microsoft Defender Antivirus or User Account Control enabled. As long as you are confident that the installation package is valid software from Avid Technology, you can click the Run Anyway button in the Windows prompt. After you select this option, Windows should not prompt you again for the same software package.
Production Management Engine Bug Number: IPI-3640. Using Interplay Access to inspect assets on the target Production Management system reveals that the Workspace value is set incorrectly. The process sets the value to the original source Workspace, and not the correct target Workspace. Additionally, if you Update Status from Media Indexer to fix the issue, the next Sync cycle overwrites the value again with incorrect data.
This fix updates the workflow so that the Workspace value is not configured for synchronized assets. The Workspace value appears blank on the target system until you Update Status on the newly synchronized assets. This change prevents the value from being overwritten with invalid data during a future sync cycle (after updating the status on the affected assets).
You must apply the update to the target Production Engine to enable this fix.
Bug Number: IPI-3693. When using some versions of MediaCentral Sync to transfer assets to a destination Production Management system, the resulting metadata for those assets on the target system might include errors. This leads to the inability to play those assets on the target system.
While the effect is seen on the target system, you must apply the update to the source Production Engine to enable this fix.
Limitations for Production Management 2024.10 Limitations for Production Management 2024.10 This section includes information on known limitations and defects related to this release of MediaCentral | Production Management.
For limitations previous to this release, see "Limitations and Defects Found in Earlier Versions" below.
...
Interplay Access Bug Number: IPI-3819. On systems running macOS Sequoia, a "No route to host" error message appears when launching Interplay Access. Clicking "Dismiss" allows you to log into Access and work normally.
Workaround: Allow Interplay Access to access local networks under System Settings > Privacy & Security > Local Network. There are instances that even if Access is already enabled under Local Network, the error message persists upon launching. In this case, you must toggle the setting off, and then on again in order for the error message to disappear.
Transfer Bug Number: IPI-3824. Performing Send To Playback (STP) to a Generic FTP profile fails for Intra-frame media (such as DNxHD, XAVC-Intra) created on Mac client workstations that contain apostrophes in their machine names.
MediaCentral Distribution Service (MCDS) Bug Number: IPI-3853. Send to Playback (STP) operations in MediaCentral CloudUX of DNxHD SQ 1080p 23.98 (DNxHD 1080 115-120-145 1 Field 23.98) media format is not supported in MediaCentral Distribution Service (MCDS).
Limitations and Defects Found in Earlier Versions For International Character Set (ICS) limitations, see the topic “International Character Support Limitations in the MediaCentral Production Management v2021.3 ReadMe.
Limitations and defects in are organized in the following sections:
Limitations for Production Management 2023.7.x This section includes information on known limitations found in Production Management v2023.7.
Interplay Access Bug Number: IPI-3602. Configuring Capture from Interplay Access only works up until 2023.7.0.
...
Limitations and Defects Found in Earlier Versions You can find this setting under: Computer Configuration > Admin Templates > System > FileSystem > Enable Win32 long paths.
Bug Number: None. If your Production Management system is configured to look at a remote CCS (Central Configuration Server) and you change the virtual host name of the local Production Management cluster, the local system might not maintain the correct CCS if you perform certain actions. For example if you change your Engine's virtual name on the same host (e.g. in case of previous temporary installation using temporary name), then you might need to complete the following workaround to resolve the issue.
Workaround: You can resolve this issue by updating a setting in the Windows registry.
1. Sign into Windows on your Engine as an administrator-level account.
If you are working in a clustered environment, sign into the current online node.
2. Click the Start Menu and type regedit to open the Registry Editor.
3. Navigate to the following key:
HKEY_LOCAL_MACHINE\SOFTWARE\Avid Technology\Workgroup\Avid Workgroup\DatabaseServer\CMS
4. If the key lists wrong virtual server name for your valid CCS, update the key with the correct value.
5. Close the Registry Editor.
6. Perform a failover to the second Engine and verify that the registry setting replicates to that server.
...
MediaCentral Distribution Service (MCDS) Bug Number: IPI-3490. Certain news sequences cannot be sent to playback AirSpeed from MediaCentral Cloud UX. The job never shows up in Production Services and fails with the error: "Remote STP Process Failed-Unknown Exception".
...
MediaCentral Transfer Engine Bug Number: IPI-3576. When sending to playback using the Generic_FTP DHM, and selecting the “create clip xml” in the Playback Device field in the TM Config, the XML file is created, but there is no attempt to FTP it to the destination.
Limitations and Defects Found in Earlier Versions Bug Number: IPI-3558. Using the Avid Interplay Production Services and Transfer Status Tool from the 2023.7_LTM Server kit and trying to get a status while in the TM Tab will fail with Java.lang.NullPointerException.
...
Bug Number: IPI-3556. When performing an SFTP, you could run into an issue where the job has failed but the status delivered back to the user is “Completed”. This can be caused by setup errors for this TM Profile in TMConfig.
In the Transfer Engine log, LibCurl will show an error:
...
The FTP Server logs will show you what the error actually was. For example: ftp [Command] Opening file '/xx/XDCAM/My_Bug1.tmp'.
...
User Management Bug Number: ICI-4992. When configuring MediaCentral Cloud UX as a User Authentication Provider in the Interplay Administrator, the Import User process will fail if MediaCentral Cloud UX is configured for OpenID-based authentication.
Workaround: Complete the following process to allow the Production Engine to import users from MediaCentral Cloud UX.
> **Note:** This process is not required if your MediaCentral Cloud UX system is configured to connect to
Windows Active Directory.
1. Sign into Windows on your Production Management Engine as an administrator-level account.
If you are working in a clustered environment, sign into the current online node (the node running the Production Engine).
regedit
2. Click the Start Menu and type to open the Registry Editor.
3. Navigate to the following key:
HKEY_LOCAL_MACHINE\SOFTWARE\Avid Technology\Workgroup\Avid Workgroup Server\Platform
4. Right-click in the right-side of the editor and select New > String Value.
5. Enter the FQDN of your MediaCentral Cloud UX single-server or cluster.
Limitations and Defects Found in Earlier Versions
6. Double-click the new key and enter the following Identity Provider ID in the Value Data field:
00000000-0000-4000-a000-000000000009
7. Click OK to confirm the change and close the Registry Editor.
8. Launch the Interplay Administrator, select the User Authentication Providers option, and do one of
the following:
- If you are configuring the MediaCentral Platform Authentication settings for the fist time,
configure the settings normally, as directed in the Production Management documentation.
- If you have already entered a value into the Avid MediaCentral Platform Servers field,
deselect the MediaCentral Platform Authentication check box, click Apply, reselect the check box, and then click Apply again to confirm the setting.
Limitations for Production Management v2022.3 This section includes information on known limitations found in Production Management v2022.3.
Installation and Configuration Bug Number: RELENG-3176. When unzipped, the Production Management installer might include long file path names that are incompatible with some versions of Windows. If you need to copy unzipped installation packages from one location to another, Windows 10 and Windows 2016 provide a registry setting that allows for these long path names.
...
Limitations and Defects Found in Earlier Versions Bug Number: IPI-2945. Service Framework has JRE 7, but Copy/Move/Archive/Restore providers are built with JRE 8. Due to this, Service Configuration tools fail with an error, and you are not able to configure the providers.
...
Transfer Bug Number: None. When launching the Transfer Engine application, an installer dialog may appear and additional update steps will be performed. Once the update has completed you will be required to reboot a second time.
...
Limitations and Defects Found in Earlier Versions Bug Number: IPI-3380. When using PWT on send to playback with DET, specifically XDAM50 jobs send from the Editor, you get in a bad state after canceling a PWT job. This leads to stuck jobs that were requiring a Transfer Engine restart to clear up and give back the stream.
...
Bug Number: IPI-3303. Large, complex sequences that are sent to playback via EVS, fail on the connect to playback service with the error message: Failed to connect to the playback service.
...
Limitations and Defects Found in Earlier Versions Bug Number: IPI-2790. Capture server clips fail to perform a Workgroup transfer unless checked out and relinked by the editor first.
...
The running jobs are both paused, and the two PWT jobs start. If the two PWT jobs take longer to transfer than what the FTP Servers connection timeout is set to, the original Transfers will timeout and fail. The connection timeout value needs to be at least 15 minutes (and possibly longer), depending or your transfer speeds and media type. This value may need to be set higher depending on your individual workflow.
...
Limitations and Defects Found in Earlier Versions Bug Number: IPI-2764. When bringing in 1080i content with logged markers, when transcoding, using MediaCentral Transcode, with a profile set for Crossrate, the markers are preserved, but the Timecode changes from Master (military) to Absolute.
