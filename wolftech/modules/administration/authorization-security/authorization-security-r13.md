...
Let´s look at how resources are organized:
Resource Classes
...
Resource Types
All of them are categorized under resource types. Each resource can belong to multiple resource types if they perform multiple roles.
Companies
...
Search by resource type, spoken languages, locations or skills. All is customizable. The more feature rich the profiles are the more fun it is.
...
Profiles
Opening their profile will reveal all the information including their details and location,
booking history:
...
In this section we will cover some of the existing functionality to provide better context for how the new functionality works.
...
Note: If you enable the booking only option that office will not be available to categorize your stories or productions or your people and will only be used as an option when making requests.
...
It is important to note however that you can set up a resource profile template for all your people by selecting the Human level and defining a template. We encourage people to do so at the very top level so you can search all your people according to the same fields for consistency:
...
- Social account handles (For on screen talent and journalists)
...
All this information will then be available in the resource profile provided that the user has finance permissions

Resource Profile
Once you have all the framework defined, the users can begin creating Resources and filling out the profile templates or editing after the fact.
...
Abilities are the type of actions your users will be able to perform and are listed on the Authorization Page. By grouping these abilities into “Roles” an admin can define a user profile that will be able to perform a combination of abilities.
A user can belong to multiple authorization roles to gain additive abilities. So, you may define a role for managing resources and another one for finance and a user could be in both.

As an Admin you can go to the admin-Authorization Page to set them up. You can name them, edit them or delete them all from the left column:
Now, you may not need to use all the abilities in the system because you don't want to restrict permissions for those actions.

In that case you can enable or disable the abilities you don´t need in the Abilities tab:
Manually Assigning Authorization Roles
...
Your system can be configured with one or multiple SSO providers (from different systems, Google and Entra ID or multiple instances of the same type) on a single database for larger organizations:

Please consult with your project manager for initial setup.

To configure the SSO groups, once a SSO provider has been connected, In the Configure SSO admin area, you can add your Groups. Do note the names need to match your organization's SSO Groups. In addition, you can add an object ID to your SSO group if mapping the object ID.
Notes:
This setting adds externalGroupId to GET & POST endpoints for group claims, which is the external identity provider’s group ID.
This new App Setting needs to be toggled on, in AWS via request to Linpro, AFTER the SSO groups have been configured with the new Ids.
...
Mapping Resource Types to SSO Groups
For a full end to end workflow from the SSO system to Wolftech you may want to automatically assign both authorization roles as well as the resource type. To do so you can enable this option in the settings section.
...
### USER GROUPS
Giving your different functional teams some permissions and categorizations is the first part to set up the right collaboration workflows for your organization.

Next, you will give them access to the right View Collections (Dashboards) and set up the right communication flows for clear and easy collaboration.
Both are possible by leveraging user groups.

So, what are user groups? A group of users can be:
- The users that belong to a department
- Those belonging to an Authorization Role, or
...
Authorization roles need to be enabled to be a user group. You can do so in the Authorization admin page by clicking on the 3-dot menu and selecting “Allow role to be reused”
