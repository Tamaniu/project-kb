...
Although Feeds shares much of the structure you are now familiar with from the Plan Module, we built it as a separate module because users need to frequently switch between Feeds and Plan views easily without having to load different collections all the time.

On another note, the Feeds view is a fast page where you can quickly create events, pitches, stories, or episodes out of a feed in seconds.

### Access

Provided a collection is shared with you as soon as you land on the Feed Module you will see the active View Name you are on in the top left (see “Eirik´s View”). Next to it you will see a dropdown icon that will expand to display the other views you have access to as part of the shared collection of views.
If you have been given access to multiple shared collections of views the system will first display your department’s default collection, but you will be able to load the other shared collections as needed. See Loading other shared collections section.

##### Loading a shared view collection
Advanced users may have access to multiple collections of views. To load a different collection to the one you are currently viewing simply navigate to Edit->Load collection
...
Note, selecting a view in the collection, on the right side (see “Internal Alerts” above), will help you navigate directly to the chosen view when you “Load” the new collection. All the other views from the chosen collection (See “Eirik´s View”) will be available now in the drop down:
...
The Feed view can now be experienced as a list view. Toggle the list view with the top right list icon:

You will then see
  - your columns listed on the left pane
  - A header in the middle displaying source, title, date, priority, category and subjects
  - all feeds belonging to the selected column listed in the middle
  - Selected feeds that you choose to expand on the right.

Note: you can use the eye icon to manage the columns you want to display

The keyboard navigation is simple and very fast.
- Tab through Columns
- Hitting the spacebar in a column will both load the column and land you on the first feed
...
- Day (Select one date and see all the selected day feed)
- Week (the last 7 days of feed from the selected date)
- Month (the last 31 days of feeds from the selected date)
Do note, Dataminr feeds will continue to show regardless of the date filter because they do not have a date (they are live events)
...
If you are not the owner of the shared view you will be able to see the filters applied to this view, but you will not be able to edit them. Instead in the 3 dot you will see an information icon:
Click on it to reveal the applied filters:

##### Tiles overview
As a general framework, feed tiles will display the source Icon on the top left, the date and time it was created, the alert threshold level on the top right, the first few lines of the report. At the bottom you will find the source link and location, subjects and or attachments if included.
Below is an example of a newswire:
...
YouTube tiles are a great way to track output on your or other YouTube channels. The tile itself, when expanded, will let you interact with the video player full functionality without leaving Wolftech.
...
Sony CI tiles will be created with every new asset added to your designated CI Hub and they will present the file type information on the tile.
...
##### Feed Searches
Users can use the search bar to further filter the results.
...
##### Quick adds
The key action out of an incoming Feed is to quickly create Stories/Pitches/Logs or Episodes out of it. To do so simply click on the + quick add button inside of the feed.
All the information from the feed will be contained in the new story.
...
In Feed List View you can bulk select multiple Feeds to link them to a story:
From Within a story, it is also possible to link feed items individually via the Hunter panel on the right. Simply search for a feed and drag it

In all cases the linked feed will show in the story Board as a linked feed:
...
Provided that you subscribe to high-res media as part of your integration with AP, EBU or Reuters, you will be able to choose to include the high-res video as an attachment when creating a story out of the feed or when linking them to the story.
The attachment will be visible in the feed card
The attachment will be visible in the quick add modal at the bottom and when added to the board the asset will be listed there with a link to the feed item:
...
This will save it in your Pins section on the bottom left.
...
all the information will also be gathered:
Including location data where available

As per regular feed items, the Dataminr event will display in the Board section:
Dataminr Feeds have the ability to group related events into one feed event. This allows you to track how a story evolves over time. You can do so via the Dataminr Panel on the right where you can see the linked dataminr events and you can open it to scroll through all the related developments:
Because these events can grow large Wolftech has added a summarize button that can provide the status of the event by summarizing all of the related events until the point you trigger the summary.
...
##### Alerts
Feeds can send automated alerts to system users depending on the severity (News Scale importance) of the feed.
In the Feed admin section, for official newswires and internal Alerts, you can define whether a certain News Scale level will send
- no alerts (none)
...
##### Printing
To share the feeds with others who do not have access to the system you can print a column by simply going to the 3-dot menu for the column
Results typically display best in Portrait mode:

To Print an individual Feed item, you can click on the print icon or simply use the keyboard shortcut P
This will print all its content, whether it fitted the feed card or was part of the read-more area.
...
Depending on your permissions you will also be able to “Share view with collection,” “Copy view to Collection” or just “Save to My Views” (from where a version can be quickly created) right inside of the feed view:
...
To create a new feed view, go to a collection and click on the “Add View” button and name your view. This will set it as a draft.
...
Once loaded, go to edit and enable edit mode to display views in draft mode.
Now that you are in edit mode you can navigate to the view you want to edit which will initially be set as in draft:
Begin to create columns with the plus button, naming a new view, selecting the column type and sources for your different columns.
...
All changes made to a shared view after publishing it will update the view for all users with access to that view in any collection where that view is present. They will see the changes the next time they open that view.
...
##### Old vs New Feed Page Access
The old feeds view page is discontinued from R14
...
Once we have added them, you can manage them in your system by going to the admin/feeds section where you can set the critical alert levels.
...
Internal Alerts
Internal Alerts are an effective way of managing internal communications that inform a user group or a department. To see them in your feeds columns you will first need to set them up in the admin / alerts / internal alerts section with your desired configuration:
Once they are set you will be able to manage them from the Admin / Feeds section:

Email Feeds
These are Feeds that add tiles per email sent to a specific email address. These are configured in the workflow engine. Please contact your project manager if you would like to configure this in your instance.
Note emails will be sanitized on import for security and compatibility.

##### Manage View Collections
Once you have all your sources and their critical levels configured, you are ready to set up Feeds View Collections.
