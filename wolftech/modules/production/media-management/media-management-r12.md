...
Wolftech runs most of its integrations through a powerful workflow engine that can communicate with both cloud and on premise systems through a single VPN tunnel making it easy to continue to do changes in the future without difficult security processes each time around.
...
### BOARD
#### Intro
Wolftech stories boast a Board to host your story related assets. This is the place where journalists can easily drop assets without the need of a PAM or MAM system. It is also an ideas board where users can also link Feed items, Links, Sticky Notes or highlight comments. Regardless of where items are added in the story the board acts as a central source of truth for all linked assets.
...
The chips below it indicate the type of attachments present in the board
...
To attach an item to the board, In the web version, you will see a + button  where you can add items in different ways:
...
To attach an item via the background information item there is an option to attach assets.
If you attach an item in a segment it will display at the bottom of the segment:
...
Finally, In the background information segments you will also be able to add board items already present in the board:
Note that you can add any number of items to a segment but you cannot add the same item to more than one segment. Items in the board that are available to be added will have their title in bold and those that are already taken will be dimmed down.
...
###### Preview
Once a file is uploaded you will see the resulting file displayed in the board. Depending on the file type the file will be previewable or just attached. See the File Extension section for more information.
...
We have partnered with the Content Authenticity Alliance to offer workflows to determine the content provenance of the files added to the board.
Content Credentials are currently supported across image, video and audio files. Not live streams. It is an industry standard that is being implemented by camera manufacturers, image and video manipulation softwares and publishing platforms so you can show to your audience the content provenance of your media from acquisition to distribution to significantly increase audience trust.
Files added to the board will have a CR watermark to display its content credentials.
If the files have any content provenance credentials we will display them on hover over the CR watermark
From it you can see the last change done to this file and you have an option to inspect the element. This will open the file in the https://contentcredentials.org/ website to check its full history.
#### Sticky Notes
...
###### Add MAM link
This action can be performed via the board + button. This is a customisation for a very specific workflow for RTL Germany not to be used by others. We will remove it from other accounts soon.
###### Feeds
...
However when you drag a feed to the story (or when you create a story from a feed)  it will appear in the board as a linked asset. Regular Feed items will display as per below:
Dataminr Feeds can also be dragged into the story and will display in the board with the list label at the top and a description of the event below and will then make the dataminr track in real time on the dataminr panel.
...
###### Important Tag
Assets can be marked as important by being part of a background information segment that is marked as important.
Note you cannot tag them as important in the board to prevent clashes with segment´s importance filters.
###### Main Image Tag
Pictures in the board can be tagged as “Main Image”.
...
Upon triggering the workflow you will be presented by an ingest metadata form to allow you set restrictions on ingest. This is optional
...
Behind the hood, beyond using the storyID we will collect the department, Story Date and user name triggering the ingest so these metadata fields can be passed to the PAM for onward workflows.
...
#### Tracking Changes
Adding or deleting items from the board will be logged in the activity log so people following the story can be kept updated with any important media contributions.
...
For admins it is easy to define which file types you would like to support in the board. This can help you secure your board of any unwanted files.
...
Afterwards you can easily delete any unwanted file extensions or check their categorisation
...
###### API Searches
In the hunter panel select MAM search.
Then select the MAM you would like to search in
...
###### Supported API-based PAM-MAMS Hunter searches
...
You can create a PAM Folder via the MAM creation button and we will add a Wolftech unique ID to your PAM folder for tracking, to prevent issues where folder file paths change over time.
You can now set the Media Storage to display all assets in your MAM that are tagged with the storyID, rather than create a collection for it.
...
An embedded experience is also possible. This allows you to leverage AI natural language processing for your searches, full access to all filters,  preview assets before import etc.
Light mode and dark mode preferences in wolftech will sync automatically with Vidispine for a unified look.
###### Linking assets
...
###### Ingest to Media Storage
Use the upload button to launch the upload dropzone and easily drag and drop assets into your collection
...
You are able to share those assets for onward review and collaboration
You can even manage all of the shares created right within wolftech
###### Editing
Set in and out points for subclipping saving them as assets or sequences for further editing
...
To switch your system to this mode go to admin - Advanced settings (MAM):
###### Ingest to Media Storage
You can ingest media from your board to Avid Media Central using the ingest to media storage action. Behind the hood, beyond using the storyID we will collect the department, Story Date and user name triggering the ingest so these metadata fields can be passed to the PAM for onward workflows.
...
###### Approval and Review
Clicking the asset in Wolftech will open a deeplink to the asset in Media Central where all of its AI capabilities will allow you to dissect the clip in real time. Tagging is performed in a Media Central separate browser.
###### Editing
Avid Media central is deeply connected to its own cloud editing functions as part of Media Central Cloud UX as well as its deep integration with Avid Media Composer and Pro Tools.
...
Create LiveU resources in your preferred resource types and select the live U workflow for them.
...
The media captured by the device will be tagged with the Wolftech Story ID.
The customer can set up a workflow to also push your MAM ID to the LiveU media so all of the clips are automatically ingested into the Wolftech created Story MAM collection (currently only supported with Mimir) If this is done, you should be able to see the media in your MAM collection as it is being recorded.
The assets will then be searchable via the hunter module and you can then drag them to your Media Storage section.
#### HAIVISION
### API AND BEYOND
#### Public API
We now have nearly 100 public APIs available for more access to some of the data and functionality in Wolftech News. Contact your account manager to get the latest documentation if you are planning on using them.
