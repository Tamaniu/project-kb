# SEARCH
**Source:** Wolftech OFFICIAL_Documentation — 01_User Experience, Navigation and Accessibility/SEARCH
**Revision:** R13
...
### GLOBAL SEARCH
The global search function is situated in the top bar for easy access
The initial search results depend on your admin configuration. It can search all types of entries in the system (All) or be configured to search a particular type (Story, as above)
Clicking on a result will open the item.
Indexing requests, stories, pub points, events and users starts immediately after they are added for faster results.
#### Permissions
Any Search in the system is bound by the permissions of the user to see the items. Ie. if you have access to certain departments, you will not see stories outside of them. Or if you have access to certain resources these will apply in the search.
#### Advanced Filtering
If you want to access the advanced search experience, click on the search category you would like to do from the dropdown menu on the right
This will take you to the advanced search page where you can filter the results further using the filters on the left
...
Below you will find custom fields defined by the admin to refine your searches.
These fields become powerful tags for filtering on the left menu of the advanced search:
...
- Use “I” to Combine terms: Show results containing either of the terms separated by the modifier - Example: Football | Soccer
...
- Use “()” to Combine modifiers in diverse ways: Use parentheses to override operator precedence. The expression inside the parentheses is executed first - Example: Influenza | flu + avian shows results containing the word influenza. It also shows results containing both flu and avian. (influenza | flu) + avian shows results containing both influenza and avian or both flu and avian.
...
Note: The fuzzy search will find matches for words that require at most 2 changes to match your search word. In the example below “Dead” will find deadly while “de” won´t find deadly as it has 4 changes to find it.
#### Feed Filtering
...
Do note that the Feeds Page allows for adding keywords and operators to each column filter you create. This is the recommended way of creating Feed Research Dashboards containing any number of searches across your feed sources. For more information on this please refer to the Feed Page guide.
#### List vs Thumbnail view
Overall, the Search results can be seen on list vs thumbnail view.
The list view will be the one that provides more information across columns, and this can be great to know you have the right element:
...
Searches for Publishing points will use the publishing date rather than its creation date (Changes to publishing dates need to index before they display which may take a minute)
...
Saved Searches will go under the My Searches Section where you can organize them in folders:
After creation you can edit the name, move them, or delete them within your “My saved Searches” section using the 3-dot menu.
#### Sharing Searches
...
If someone else shares a search with you, you will find it in the Shared Searches section from where you can organize them under folders
...
Do note that while everyone can save a search, you will need the right permission to share searches, so please contact your administrator if the option is not available.

#### Mobile
Global Search is available through mobile experience. Simply navigate to the hamburger menu on the right and select Search. This can be immensely powerful on location, not only for finding stories and quickly accessing them but also to find people's profiles to quickly contact them or finding points of interest for your project.

### HUNTER
Hunter is a quick search module nested inside of your content right panel to search for items across the application and link them to your content. Find something important and simply drag the find to your story central panel to associate them.
In Hunter you can search for
- Events
- Feeds
- Stories
- Episodes
- Read-in Notes
- Connected MAM assets

Hunter does not currently offer filters for your searches but leverages the same searching reach as global search (Ie it will search titles, descriptions, and searchable custom fields).
For MAM asset search see the Hunter Media Search section below.
...
When using this function bear in mind the system caches all story changes every 3 minutes so results won´t be instantaneous.
...
#### Hunter Media Search
When  you need to look for media for your story but outside of your story assets you can leverage the Story Hunter (See Hunter for more info) where you have access to search across all of your connected media systems to later add the link the media to your story by drag and drop.
The Hunter provides filtering options custom made for your connected system and installation:
...
When you find a match, we display a summary of the story and whether the story has been shared with other departments, via the split color highlight, directly inside of the find similar stories navigation, once you click on the story you are interested in.
...
The same is possible when categorizing resources in the resource type profile templates:
To manage the permissions of who can share saved searches go to your authorization and create a role with the correct permissions:
To manage shared searches that may need removing after a colleague has left the organization admins can go to the admin - Saved Searches Menu:
Here you will be able to unshare those searches so that they are no longer visible to current users.
