...
Avid NEXIS® Administration Guide Version 2024.10.0 Using This Guide Using This Guide NEXIS® The Avid software-defined storage system provides a high-performance distributed file system that uses high-capacity shared media storage for workgroups of connected devices. This guide describes how to manage your storage system.
...
> **Note:** To avoid being logged out automatically, see "Avoiding Automatic Logout" on page 21.
...
Menu Selection, Displays the name of the selected menu item, and action tools (icons) for available actions Messages, and based on the objects being displayed. See "Action Tools" on page 19.
...
Cloudspaces, including the subscription plan and status and the amount of storage and download data included in the plan.
...
Only System Admins can run the System Monitor. (For more information about System Admins, see "Adding Users to the System Administrators Group" on page 95.) To run the System Monitor View:
...
(Listed in alphabetical Order) name Cannot start or end with a period Can include spaces Team name 31 No Unicode upper- and lowercase Cannot start or end with a period Can include spaces g A Team prefix (if used) counts toward the Team name limit.
...
Workspace name 31 No Unicode upper- and lowercase Cannot start or end with a period Can include spaces (unless prevented by Team Policy; see "Managing Avid NEXIS Teams" on page 100) g A Team prefix (if used) counts toward the Workspace name limit.
...
See "About Avid NEXIS Subscriptions and Licenses" on "Binding Media Packs to the File System" on page 40 Yes "Configuring the Notification Service" on page 42 Recommended "Setting the System Time" on page 42 Strongly recommended for licensed systems; see "Licenses and System Time" on page 29.
"Changing the Default System Passwords" on Recommended "Securing the Avid NEXIS Infrastructure" on page 1 Recommended "Configuring SNMP Monitoring" on page 48 No "Enabling Link Aggregation" on page 49 No Creating user accounts and groups: see "Managing Yes (for users; groups are optional) Creating Avid NEXIS Teams; see "Managing Avid No "Creating Storage Groups" on page 56 Yes "Creating Workspaces" on page 61 Yes
> **Note:** After changing a configuration, always open a new browser tab and log back into the Agent or
Management Console before using it again. Avid recommends this to make sure the browser cache is clear.
...
- Client Manager ("Configuring External Authentication of Users" on page 85)
To configure DNS:
...
About Trusted Certificates A trusted certificate is issued by a recognized certificate authority. Organizations can even create their own trusted certificates. Trusted certificates ensure that the URLs for an Avid NEXIS system are validated, that everyone is who they say they are.
...
Administrators user group) can apply a certificate. See "Applying a Trusted Certificate " on page 36.
...
2 Configuring and Licensing the System Trusted certificates typically expire after one or more years (controlled by the issuer). The system
- displays warnings at 60, 30, and 7 days before expiration, and the messages change color at each
interval. If you configure email notifications, warnings are also sent by email (see "Configuring the Notification Service" on page 42). You can view the trusted certificate expiration date on the System > System Setup > Security tab.
...
DNS domain name where All the Avid NEXIS system is registered 2 Configuring and Licensing the System Required Information Applicable Notes Platforms List of DNS servers and On-premises For more information see "Configuring DNS Service " on The certificate authority provides you with the following:
- A certificate file (can have the extension .crt, .cert, or .pem)
- A key file (typically with the extension .key)
Applying a Trusted Certificate When you receive your certificate and public key files, make sure they are decrypted, then upload (or copy and paste) them and apply them to the Avid NEXIS system.
...
g You can click and drag the bottom right corner of the Certificate Content field to enlarge it even more.
...
Exporting an Avid NEXIS Self-Signed Certificate You can use the Avid NEXIS self-signed certificate to establish a secure connection to the Avid NEXIS system. To export a copy of the certificate, launch the Management Console from any web browser.
> **Note:** The export procedure may vary depending on the operating system and browser type. The following
steps outline the process using Google Chrome on Windows.
...
E2 All-Mirror Y E4 All-Mirror Y E5 All-Mirror Y E5 Nearline N PRO N F2 Y F2X Y F5 Y F5 Nearline N PRO+ N High Performance E2 N (1024KB strip size) E4 N 2 Configuring and Licensing the System Bind Option Available On Media Mirroring Supported?
Intended for demanding workflows with low client counts (10 or PRO N fewer).
F2 N F2X N PRO+ N SSD E2 SSD Y Intended for ultra-high bandwidth media playback. F2 SSD N Supports Media Mirroring only with the E2 SSD.
...
To set system time, or configure an NTP server:
...
2 Configuring and Licensing the System You can apply filters to include or exclude messages from the following subsystems based on the severity level (Error, Warning, and Informational):
...
c. In the Email Address field, type an address for the sender of the email messages. This will be the name and address in the “From:” field of all email messages sent from this system; for example, “Ace NEWS NEXIS.” (By default, email messages also contain the IP address and host name of the system that sent them.) d. Click Save.
...
premises only). See "Changing the BMC Password" below.
...
2 Configuring and Licensing the System Manufacture Applicable to Default password Date Before April 1, Avid NEXIS E-series, Avid NEXIS | se-admin 2020 PRO, System Director Appliance On or after April 1, Avid NEXIS E-series, Avid NEXIS | Last 8 characters of the Controller serial number (on 2020 PRO 40TB, System Director the label) Appliance For example, if a Controller has the serial number RMS1005576G008Y, the default password is 576G008Y.
...
History Data check box Enabled When enabled (default), collects system history data Collection (see "Displaying Event History" on page 134. If disabled (for support use only), the system stops collecting new data, but existing data is retained.
...
To verify or monitor the status of link aggregation on the controllers, see "Displaying Controller Information" on page 122.
...
You can increase the amount of space in a Storage Group at any time, with some exceptions. For more information, see "Adding Unassigned Media Packs to Storage Groups" on page 114.
...
- Some Workspace move operations ("Moving a Workspace" on page 73)
- Media Pack or Engine removal in some cases (see "Removing Media Packs from a Storage Group" on
page 116) To be (or become) mirror capable, a Storage Group:
...
Creating Storage Groups To create a Storage Group the system must have at least one bound Media Pack. See "Binding Media Packs to the File System" on page 40 and "Storage Group Performance Options " on page 52 for more information. Ideally, create Storage Groups from Media Packs of the same size (capacity).
To use media mirroring (on-premises only), the Storage Group must be mirror-capable or All-Mirror. See "Protecting Workspaces from Engine Failure " on page 52.
...
3 Managing Storage Groups Changing a Storage Group’s Capacity A Storage Group’s capacity is the product of the smallest Media Pack in it, multiplied by the total number of Media Packs in that group. For more information, see "Storage Group Options" on page 51.
You can change the Storage Group’s total capacity as follows:
Add more Media Packs ("Adding Unassigned Media Packs to Storage Groups" on page 114).
- Remove one or more Media Packs, within certain restrictions. See "Removing Media Packs from a
- Storage Group" on page 116.
Replace smaller Media Packs with larger ones through a series of add and remove operations, which
- trigger Workspace redistribution (see "About Workspace Redistribution" on page 74). When all the
...
Mirror Capable Displays whether the Storage Group supports Engine protection on Workspaces in the group (see "Protecting Workspaces from Engine Failure " on page 52). Values are Yes or No.
...
means that no Team quotas have been set. ) Files Displays the number of files in all Workspaces in the Storage Group Directories Displays the number of directories in all Workspaces in the Storage Group 3 Managing Storage Groups To see or hide the Media Packs in a Storage Group:
1. In the navigation sidebar, click Storage, then Storage Groups.
...
Deleting Storage Groups An administrator can delete a Storage Group if it does not contain any Workspaces or Team quotas. (See "Deleting Workspaces" on page 77 and "Deleting a Quota" on page 108).
...
Workspaces can have their own access privileges, permissions, protection modes (on-premises only), and event triggers. At least one Storage Group must exist before you can create a Workspace; see "Creating Storage Groups" on page 56.
See "Groups and Roles at a Glance" on page 81 for information on which user types can perform Workspace operations.
...
System Admin) or the Team Admins can create the Workspaces they will need. See "Managing Avid NEXIS Teams" on page 100 for more information.
...
For users who do not belong to a Team, you can apply access permissions at the Workspace level (see "Managing User and Group Access to Workspaces" on page 70), and on folders within the Workspaces through a set of application programming interfaces (APIs) available for download from Avid (only supported for the default administrator; see "Appendix — Managing Folder Level Permissions" on page 159).
...
> **Note:** This section does not apply to Avid NEXIS | Cloudspaces. Depending on the Team policy, some protections options might be enforced or not available to Team
Admins.
...
- restrictions of any policy for your Team (see "Your Team Properties" on page 110).
...
field. (If there is not enough space, you can reduce the size of existing Workspaces to make more space available. See "Resizing Workspaces" on page 71.)
...
next field.
> **Note:** For more information, see "Setting Event Triggers " on page 72. Event triggers send email to the Avid NEXIS administrator when a Workspace reaches the
...
> **Note:** As you increase the size of a Workspace, the Maximum value for all Workspaces in
the same Storage Group decreases.
...
- value, type a number in the Capacity field instead. See "Resizing Workspaces" on page 71.)
...
Managing User Access Privileges for Workspaces By default, new user accounts do not have access privileges to Workspaces until an administrator assigns them (see "Creating (Adding) Users" on page 83). You can customize access for each client to every Workspace, if necessary.
...
- At the user group level—change the group attributes to give all members of the group the same
access to one or more Workspaces 4 Managing Workspaces Understanding Access Privilege Combinations The permissions set for users, groups, and Workspaces work together to grant the highest level of privilege of the combination. More privilege (such as read-write) supersedes less privilege (such as read-only).
...
page 91. To control access to a Workspace for all users and groups, see "Managing User and Group Access to Workspaces" on the next page.
...
Managing User and Group Access to Workspaces In addition to managing specific users’ or groups’ access to Workspaces through the Users and Groups menus, you can also change the properties of the Workspace itself to grant or restrict access for multiple users and groups in one operation. (See also "Changing User Properties" on page 91 or "Changing User Group Properties" on page 96).
> **Note:** Make sure you have permission to perform this operation. See "Groups and Roles at a Glance" on
page 81 for information on user account types and what each type can manage.
...
The Storage Group must contain at least as much available space as the amount by which you want to increase the Workspace. If not, you can add more Media Packs to the Storage Group first; see "Adding Unassigned Media Packs to Storage Groups" on page 114.
...
> **Note:** See "Groups and Roles at a Glance" on page 81 for information on user account types and what
...
4 Managing Workspaces If the original Workspace used no Media Pack protection (Unprotected), you must select either One
...
If you enabled notifications, you receive email when Workspace capacity events occur. For more information, see "Configuring the Notification Service" on page 42.
...
- Storage Groups (see "Managing Storage Groups" on page 51).
...
- moving into it, also accounting for any Team quotas. Check the Capacity Tracker to make sure the
Storage Group has enough space to allow the move. You can add more Media Packs to the Storage Group first; see "Adding Unassigned Media Packs to Storage Groups" on page 114.
...
- The Workspace can remain in use during the move operation — clients do not have to unmount or stop using
it.
4 Managing Workspaces Information for Team Administrators If you have quotas in multiple Storage Groups, you can move Workspaces between compatible Storage Groups if your Team policy permits it.
...
4 Managing Workspaces A configuration change is made. On the Workspaces list page, the Config Changes count for the
...
Packs.
Deleting and moving Workspaces results in a redistribution but does not cause a configuration change. You can see the number of configuration changes that have been applied to a Workspace; see "Displaying Workspace Information" on page 62.
...
4 Managing Workspaces Data loss can occur if you do any of the following:
- Remove or unbind a Media Pack before redistribution is complete - Remove more than one Media Pack from a Storage Group before performing a redistribution - Turn off any Engine before redistribution is complete Performing a Full Redistribution In addition to automatic redistribution operations (see "About Workspace Redistribution" on page 74), you can also initiate redistribution manually, for example to perform a full redistribution when the Config Changes count exceeds 20. (For information about Config Changes, see "Displaying Workspace Information" on page 62.)
...
5. Resume redistribution when appropriate. See "Resuming Redistribution" on the next page.
4 Managing Workspaces Resuming Redistribution If you suspend a redistribution, make sure to resume it. Do not leave Workspaces in the suspended state for extended periods of time.
...
fail. See "Understanding Media Pack Status" on page 113.
...
Glance" on page 81 for information on user account types and what operations each type can do.
...
5 Managing Users and Groups 5 Managing Users and Groups A user is a person or device (such as a client computer) that needs to access an Avid NEXIS shared-storage network. User accounts on Avid NEXIS allow this access and prevent unauthorized users from accessing other users’ data. You can create users manually, and you can configure external authentication to allow LDAP users to connect to Avid NEXIS. To import LDAP users, see "Importing Users from a Server " on page 84.
...
You can also create Avid NEXIS Teams, which have their own users and groups; for more information, see "Managing Avid NEXIS Teams" on page 100.
...
the Workspace access privileges for all or many group members, use the Groups page in Edit mode (see "Changing User Group Properties" on page 96).
To change the access privileges to a specific Workspace for multiple users or groups, use the
- Workspaces page in Edit mode (see "Managing User and Group Access to Workspaces" on page 70).
In addition to a name and password, users can have optional flags that control what they can do or how they are authenticated; see "About User Flags" on page 82. You can also control the bandwidth available to a user; see "Controlling Client Bandwidth" on page 98.
...
The Avid NEXIS system maintains a record of all connected clients (users or devices). See "Monitoring the System" on page 129 for more information.
...
group has a role called System Admin. That role Members of this group can use the APIs (see "Appendix — Managing cannot be applied to any Folder Level Permissions" on page 159).
...
For more information on Teams, see "Managing Avid NEXIS Teams" on page 100.
Managing Users You can create, modify, and delete user accounts, assign users to specific groups or Teams, and manage access privileges for all users by the individual user, by user groups, and by Workspaces. Some user groups have additional capabilities (see "Understanding Groups and Group Roles " on page 79). You can add users 5 Managing Users and Groups to groups with those roles to grant the users those privileges, and you can apply roles to an existing group to give all the members the same privileges.
...
- Bandwidth limit (optional; for more information, see "Controlling Client Bandwidth" on page 98)
- Team membership (optional; for more information, see "Managing Avid NEXIS Teams" on page 100)
...
6. In the User Flags section, click the appropriate boxes (see "About User Flags" on page 82 for more
information).
7. In the Group Membership list, select a group and click Member to add the user that group. By
default, new users are not members of any group. (Some groups might not be selectable; for example, if you add the user to a Team, only that Team’s other groups, if any, are selectable.) The Group Membership panel shows the number of groups available on the system, the number currently selected (if any), and how many groups the user currently belongs to. As you select more groups, the selected value increases, but the Current membership count does not change until you click Save. (See the Avid NEXIS ReadMe for the limit on the number of groups a user can belong to.) The user inherits the roles of any group you add them to (see "Understanding Groups and Group Roles " on page 79).
8. In the Workspace Access list, select one or more Workspaces and click one of the following access
types:
(Some Workspaces might not be selectable; for example, if you add the user to a Team, only Team Workspaces are selectable.) By default, new users have no access to Workspaces. (For information on permissions for Workspace access in the network, see "Managing User Access Privileges for Workspaces" on page 64.)
9. Click Save.
...
Prerequisites To enable using the LDAP Sync Tool, enable DNS and configure external authentication on the Avid NEXIS system. For more information, see "Configuring DNS Service " on page 27 and "Configuring External Authentication of Users" below If you are using Avid NEXIS Teams (see "Managing Avid NEXIS Teams" on page 100 for more information), you can add the user names for the intended Teams to your authentication server and then import them.
...
Configuring external authentication and using the LDAP Sync Tool is optional, but recommended for large numbers of users. Alternatively, you can create each user account individually; see "Managing User Groups" on page 94. You can use both external authentication and accounts you create on the Avid NEXIS.
...
and Avid NEXIS Accounts" on page 89.
...
> **Note:** LDAP supports having users with the same name but in different OUs, but Avid NEXIS
does not check OUs.
...
- User flags (see "About User Flags" on page 82)
- Group membership
- Workspace access
...
- Click Refresh.
- Click Edit.
Disabling and Enabling Users You can disable and enable a user account any time (within the restrictions described in "Changing User Properties" on page 91).
...
6. Click Save.
Deleting Users You can delete a local or remote user account at any time (within the restrictions described in "Changing User Properties" on page 91).
...
want members of the new group to have. (If you selected a Team in the previous step, the Group Roles become unavailable.) See "Understanding Groups and Group Roles " on page 79 for more information.
...
If you selected a Team, only Team Workspaces are selectable. Unless overridden on a per-user or per-Workspace basis (see "Managing User and Group Access to Workspaces" on page 70), all the group members have the same access to any given Workspace.
8. Click Save.
...
5 Managing Users and Groups more easily apply a different access type to others.
...
Changing User Group Properties You can rename a user group, change its user membership, change group roles, and modify its users’ Workspace access at any time, however you cannot:
...
- Roles " on page 79)
- User Membership — Lists all the users on the system and how many groups they belong to
- Workspace Access — Lists the access granted to all group members for each Workspace
Changing a group’s Workspace access takes effect immediately and can disrupt a workflow. Avid recommends that group members unmount a Workspace in the Client Manager before you change their access to it (including changing their group membership if that also changes their Workspace access).
...
For information on permissions for Workspace access in the network, see "Managing User and Group Access to Workspaces" on page 70.
...
- If the group has roles assigned (Workspace Admin or User Admin or both), the group members lose those
attributes when you delete the group.
...
3. Click Delete.
4. In the dialog box, click OK.
5 Managing Users and Groups Controlling Client Bandwidth Some client applications try to use as much bandwidth as possible (for example, Avid AirSpeed servers), potentially throttling other users of the system. To control this, you can set the maximum bandwidth a client can use. See "Changing Bandwidth Limits" on the next page.
If the combined bandwidth requirement of connected components equals the system limit for bandwidth use, some clients might not be able to connect to the shared-storage network. To check current system bandwidth usage, see "Displaying Client Bandwidth" on the next page.
Adding Bandwidth Limits Avid NEXIS supports bandwidth limits you create and those created automatically when a client connects to the Avid NEXIS for the first time. You can also prevent the automatic creation of client bandwidth entries by changing the system preferences; see "Changing the Default System Settings" on page 47.
...
For automatically created entries, the default bandwidth is determined by the system preference setting called Default Bandwidth for Auto Created Devices (MB/sec). The default value of zero (0) means the client has no bandwidth limit. See "Changing the Default System Settings" on page 47 for more information.
> **Note:** The Management Console displays only the client-to-server (client to Avid NEXIS system) bandwidth
(see "Displaying Client Bandwidth" on the next page). For unmirrored Workspaces, the bandwidth displayed reflects the actual bandwidth used for writes. For mirrored Workspaces, the actual bandwidth is twice what is displayed for writes, because the bandwidth is occurring between the Media Packs and the server for the mirrored writes, not between the client and the server.
...
5 Managing Users and Groups Displaying Client Bandwidth The Dashboard displays system usage in MB/sec for both reads and writes. You can toggle the display to show reads only, writes only, or both. The Dashboard displays the bandwidth of each connected client, with separate columns for reads and writes.
> **Note:** The Management Console displays only the client-to-server (client to Avid NEXIS system) bandwidth
(see "Displaying Client Bandwidth" above). For unmirrored Workspaces, the bandwidth displayed reflects the actual bandwidth used for writes. For mirrored Workspaces, the actual bandwidth is twice what is displayed for writes, because the bandwidth is occurring between the Media Packs and the server for the mirrored writes, not between the client and the server.
...
Only System Admins (see "Understanding Groups and Group Roles " on page 79) can create, modify, and delete Teams.
...
ACE-Admin1 Team Required — A unique user account that you must Administrator create when you create the Team.
User This user can manage the Team and create more users, groups, and Workspaces for the Team. The initial Team Administrator User automatically 6 Managing Avid NEXIS Teams Attribute Description Example belongs to the Team Administrator Group.
...
If you plan to import users, configure External Authentication on the system (see "Configuring External Authentication of Users" on page 85) and add all the users for each Team to your authentication server.
Then import the users with the LDAP Sync Tool and add them to the respective Teams (see "Importing Users from a Server " on page 84 and "Adding Remote (Imported) Users to an Avid NEXIS Team" on page 106 more information).
...
the restrictions in "Moving a Workspace" on page 73. For example, if the system has both Scale-Out and SSD Storage Groups, you can allow the Team Admin to move Workspaces from one to the other for high compute operations such as rendering.
...
See "Creating (Adding) Users" on page 83 to create users manually and add them to Teams. See "Importing Users from a Server " on page 84 to import users with the LDAP Sync Tool.
...
Workspaces, see "Changing User Properties" on page 91.
...
Create the users first, then optionally organize them into Groups: see "Managing Users" on page 81 .
...
system disks, see "Displaying Drive Details" on page 116.
...
You can add bound Media Packs to an existing Storage Group with other compatible Media Packs (with the and "Storage Group Options" on page 51 for restrictions.
8 Managing System Components Adding and removing Media Packs redistributes files in the Storage Group across the currently available Media Packs. For more information, see "About Workspace Redistribution" on page 74.
...
Redistribution is not required until 30 configuration changes are recorded, in which case the redistribution occurs automatically. You can suspend and resume the redistribution as needed. See "Suspending Redistribution" on page 76.
...
Redistribution" on page 74).
- The Storage Group will have less than either the used space or the allocated space. To make enough
free space, you can delete files from the current Workspaces, or reduce or delete Workspaces themselves. See "Resizing Workspaces" on page 71 and "Deleting Workspaces" on page 77.
...
Disks table.
When an SSD reaches a Warning or Critical threshold the system sends notification to all configured contacts (see "Configuring the Notification Service" on page 42) and the Management Console displays the status.
...
See "Identifying the Slot Number for a Failing or Failed Drive" on the next page, and see the Avid NEXIS Setup and Maintenance Guide for instructions on replacing a drive.
...
Spare Media Pack The drive is a spare. To see all the drives in an Engine, see "Displaying drives only Drive Details" on page 116.
...
You can also make the drive LEDs blink to help you find it physically in your equipment room. See "Displaying Engine and System Director Appliance Details" on page 120.
...
drives.
Removing a Media Pack from an on-premises Storage Group starts a redistribution of all its data to other Media Packs in the same Storage Group. For more information, see "About Removing Media Packs from Storage Groups" on page 115. When the Media Pack has been removed, its state changes to Spare - Bound.
...
Redistribution can take a long time. Open the Redistribution Monitor tool to track the progress and estimate the time remaining (see "Monitoring Redistributions " on page 133).
...
If you move drives from one Engine to another, they are reported as foreign. To use the drives in the new Engine. clear the Foreign Disk Error on each drive (see "Clearing a Foreign Disk Error" on page 118).
...
Avid NEXIS Displays the Avid version installed on the Controller Version USM Version Displays a value for the suite of component firmware versions (for more information, see "Displaying System Component Versions" on page 130) Controller Serial number of the Controller Serial Number Engine Serial Serial number of the Engine in which the Controller is installed 8 Managing System Components Heading Description Number Free Memory The amount of available memory on the Controller Total Memory The total amount of memory on the Controller To see detailed information about a Controller:
...
> **Note:** To upgrade client systems, see the Avid NEXIS Client Manager Installation and User’s Guide on the Avid
NEXIS Documentation Knowledge Base.
...
> **Note:** Make sure all the engines that will belong to the same shared-storage system have the same
Storage System Name.
4. In the dialog box, click OK.
...
8 Managing System Components If any Production Management Media Index services are connected to the Avid NEXIS, place them into standby mode until after all the IP addresses are changed and the entire shared-storage system has finished rebooting.
...
8 Managing System Components Restarting the System If you have completely shut down an on-premises Avid NEXIS system, follow this procedure to start it again.
...
9 Monitoring the System To download the status history:
- Click Download.
...
9 Monitoring the System Filtering Media Pack Logs You can turn certain log levels on to capture specific information as requested by Avid Customer Care.
...
9 Monitoring the System To disable Media Pack logs:
1. In the navigation sidebar, click Logs, then Media Pack Logs.
2. Click the Logs tab if not already selected.
...
This feature is enabled by default but can be disabled from the Settings page in the Avid NEXIS Management Console.
...
Collecting the Delete Logs Delete logs are collected using the "Log Aggregator Tool" on page 144. Only the Administrator and System Administrators have access to collect the file deletion logs.
To collect the delete logs:
1. Open the Log Aggregator Tool. See "Using the Log Aggregator Tool" on page 145.
2. Click Configure
...
Viewing the Delete Logs The log aggregator output includes the Delete log and Admin log files.
...
g The date field does not display the dates until you resize the column, double-click the cell, or select the cell and click the formula bar.
...
g To find out which specific administrator deleted the workspaces, check the Admin log.
9 Monitoring the System Admin Log The Admin log file tracks deletions made by the Administrator and System Administrators, including workspace deletions.
> **Note:** For security, always use the System Administrator group to assign unique credentials to administrators
and to enable per-admin logging of actions.
...
compatible program.
The delete log file is named 'AvidNEXISdeleteLog-date-time.csv' and the Admin server log is named 'admin_tool_log.csv' Individual log files are limited to ~4-10 MB in size. Up to 100 log files are kept then the oldest files are deleted.
...
Avid Customer Care personnel can use this information, coupled with the system logs collected by the Log Aggregator tool (see "Log Aggregator Tool" on page 144), to diagnose and troubleshoot problems.
The following information is collected:
- System Director name (host name), storage system name, the date the snapshot was collected, and
the name of the zip file containing the snapshot. The System Director name is the name of the Controller running the System Director service.
...
9 Monitoring the System To see a snapshot:
...
9 Monitoring the System Deleting One or All Snapshot Archives To delete a snapshot archive:
...
Display Transfer Opens the Transfer Preferences dialog box. See "Transfer Preferences" on page 155.
...
Media Pack Protection Mode Avid NEXIS shared-storage systems support Media Pack protection, which protects the Workspaces from the failure of one or more drives in a Media Pack. See "Workspace Protection Methods" on page 60 for more information. When selecting the Workspaces to migrate, you can apply any Workspace protection method supported by the destination Storage Group.
...
To copy data:
1. Open the Data Migration Utility. (See "Connecting to the Source and Target Systems" on page 150.)
...
page 81 for information on user account types and what operations each type can do.
