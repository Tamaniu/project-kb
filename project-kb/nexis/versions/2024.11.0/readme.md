...
Revision History Release Date Version Changes Nov 12, 2024 2024.11.0 (Revised Updated the 2024.2 release notes to add support for March 11, 2025) port-bonding on MCCUX systems. See "What’s New in Avid NEXIS 2024.2.0" on page 12 Feb 27, 2024 2024.2.0 (Revised Mar 5) See "What’s New in Avid NEXIS 2024.2.0" on page 12 Contents What’s New in Avid NEXIS 2024.2.0 12 USM Versions 13 Hardware and Software Requirements 15 Supported Upgrade Paths 15 System Director and Client Version Compatibility 15 Compatibility with Other Avid Products 19 Third Party Editor Compatibility 19 System Configuration Limits 19 System Performance (On-Premises) 22 Known Issues 25 Special Notes 30 Accessing the Online Documentation The Avid online documentation contains all the product documentation in PDF format. You can access the documentation from the Knowledge Base site specific to your release. Download and install Acrobat Reader before you access the PDF documentation.
...
Avid NEXIS System (Server) and Client Releases Product Version Guidance Server and 2024.11.0 Recommended for all Avid NEXIS systems and clients (On-premises and Clients Cloud).
Native Apple Silicon support; Support for on-premises Rocky Linux clients; Support for 600 active clients in the Avid NEXIS | Cloud Storage system; Bug fixes.
...
What’s New in Avid NEXIS 2024.11.0 Product Version Guidance
- Avid NEXIS | F2 SSD Engines
- All Avid NEXIS | EDGE sites
Avid NEXIS versions 2022.x and 2021.x are no longer recommended for new installations. Avid NEXIS version 2020.x and older are no longer supported.
...
What’s New in Avid NEXIS 2024.11.0 Avid NEXIS version 2024.11.0 is a major release that includes the following:
- Native Apple Silicon support for the Avid NEXIS Client
- Support for on-premises Rocky Linux clients
- CentOS Support Update
- Avid NEXIS | Cloud Storage system support for 600 clients
Native Apple Silicon Support Avid NEXIS 2024.11.0 introduces full native support for Apple silicon-based clients. Previously, the Avid NEXIS Client software required Rosetta 2 to translate Intel-targeted code to run on Apple Silicon Macs. In this version, the Avid NEXIS Client software now runs natively on Macs with Apple Silicon processors (M- series).
Support for On-premises Rocky Linux Clients with 1 and 10GbE Network Adapters Avid NEXIS version 2024.11.0 introduces support for on-premises Rocky Linux clients with 1 and 10GbE connections; see"Supported Client Operating Systems " on page 16 for more information. To install the Avid NEXIS client software on Rocky Linux, refer to the Avid NEXIS® Client Manager Installation and User’s Guide available on the Avid NEXIS Documentation Knowledge Base.
> **Note:** The Rocky Linux Client kit supports command-line operations only.
CentOS Support Update CentOS Linux 7 reached end of life (EOL) on June 30, 2024. While the CentOS installers remain available on the Management Console Installers page and in the software kit, they will transition to legacy status and be removed in future releases. Avid NEXIS will continue to support CentOS clients and CentOS VFS Gateway until further notice. To ensure continued support and compatibility, please plan and schedule your migration to Ubuntu or Rocky Linux clients accordingly.
Avid NEXIS | Cloud Storage System Support for 600 Active Clients This release increases the capacity of the Avid NEXIS | Cloud Storage system to support up to 600 active clients. See the "System Configuration Limits" on page 19 for more information.
What’s New in Avid NEXIS 2024.10.0 Server Fixes (in Numerical Order) Includes all fixes from previous patches and releases.
- SSENG-35821: The Datagram Reception/Transmission information for the Avid NEXIS | F2 does not
display the correct Media Pack network statistics; the values always show zero and the System Director connection status shows "Not connected/No Session".
SSENG-39462: Fixed an issue in Avid NEXIS PRO and PRO+ systems where the error message
- "ProfileManagerLib:0 Slot 0 is not configured for redundancy" persistently appears in the System
Logs.
- SSENG-39529: When accessing the System Logs, this message appears in the log entries:
"AuthenticateFromCache: failed to authenticate against the cached password: Incorrect password". This issue is fixed by reducing the log level of the message from Warning to Info.
SSENG-41106: After receiving a Full Disk error from a media pack that had run out of disk space, the
- Storage Manager lost connection and did not restart correctly via automated method on the
controller.
- SSENG-41355: An Avid NEXIS Teams Quota cannot be adjusted if the Team Name and the Team
Administrator Group are the same but have different letter case or capitalization.
- SSENG-41500: When deleting a team workspace using the Team Admin account, the workspace
name is not recorded in the AdminToolLog.
SSENG-41397: When sorting user access in the Workspace page of the Management Console,
- clicking the Access column sorts by the Effective Access column instead. The sort arrow appears on
the Effective Access column header when the Access column is clicked.
- SSENG-41765: Resolved an issue writing to an Avid NEXIS mirror-protected workspace during
engine controller switch over.
Client Fixes SSENG-39564, SSENG-35653, SSENG-40469: Avid NEXIS clients experience kernel panics when
- used with newer versions of CrowdStrike (7.14–7.18).
SSENG-40694: Resolved a logging issue with the Avid NEXIS Client on MediaCentral | Cloud UX,
- where the System Director logs were continuously filling up, causing performance and media offline
issues.
- SSENG-40859: Avid NEXIS client versions later than 2021.8.x fail to work on some Azure-based Avid
NEXIS | Cloud Storage systems. With this fix, customers can upgrade their Avid NEXIS Client to the latest version and not experience issues on their Azure VMs.
Limitations and Defects Found See the "Known Issues" on page 25 for all limitations found in this release and previous releases. New limitations are marked as 'New'.
...
What’s New in Avid NEXIS 2024.6.2 Support for Windows 11 Build 23H2 In this release, Avid adds support for Windows 11 build 23H2, which is now the recommended build for use with Avid NEXIS client systems. Windows 11 builds 22H2 and 21H2 are no longer recommended, as they no longer receive updates from Microsoft. See "Supported Client Operating Systems " on page 16.
...
Client Fixes SSENG-40043: Resolved a compatibility issue causing a kernel panic during playback between Avid
...
What’s New in Avid NEXIS 2024.6.1 Server Fixes (in Numerical Order) SSENG-40856, SSENG-41009: When upgrading to version 2024.5.0 or 2024.6.0, the Avid NEXIS
- controllers enter a reboot loop due to hostname resolution taking too long with DNS (Domain Name
System).
- SSENG-41023: Resolved the issue that required the installation of Avid NEXIS | VFS 2024.6.0 before
installing 2024.6.1. With this fix, customers can upgrade directly from any earlier version to the latest release without first installing 2024.6.0.
Limitations and Defects Found See the "Known Issues" on page 25 for all limitations found in this release and previous releases. New limitations are marked as 'New'.
...
Server Fixes Updated OpenSSH module to enhance reliability and security.
- Client Fixes
SSENG-40566, SSENG-40740: Restarting the Avid NEXIS Agent using the systemctl restart
- avid-nexis-agent
command on an Avid MediaCentral | Cloud UX server causes a disconnection of the gateway pod. As a result, the MediaCentral | Cloud UX user interface becomes unresponsive, preventing users from logging in.
...
What’s New in Avid NEXIS 2024.6.0 Avid NEXIS version 2024.6.0 includes the following bug fixes.
...
What’s New in Avid NEXIS 2024.5.0 What’s New in Avid NEXIS 2024.5.0 Avid NEXIS version 2024.5.0 is a major release that includes the following features and improvements:
...
documentation Connections using HTTP No Longer Supported This release enhances security by changing the communication protocol from HTTP to HTTPS.
Management Console connections using HTTP through port 80 are no longer supported; only HTTPS connections (port 443) are allowed.
> **Note:** This change also restricts the use of HTTP for API services and may affect workflows on Avid NEXIS
EDGE and Avid MediaCentral | Panel for 3rd Party Creative Tools. Please refer to the Avid NEXIS EDGE 2023.12.1 ReadMe and to https://kb.avid.com/pkb/articles/en_US/Knowledge/Avid- NEXIS-greyed-out-in-MCCUX for more information on this issue.
...
The Media Pack Bind dialog box now includes a link to the Licensing page.
...
What’s New in Avid NEXIS 2024.5.0
...
What’s New in Avid NEXIS 2024.5.0 For F-series Engines with a single (failed) Controller do the following:
...
Server Fixes (in Numerical Order) SSENGHW-122, SSENGHW-123, SSENGHW-124: On Avid NEXIS | PRO+ and any Avid NEXIS F-Series
- with a single Controller, the amber Module Fault LED on the Controller remained lit, which normally
indicates an error condition. However the system did not report an error and behaved normally.
...
- SSENG-39054 In some cases, attempting to delete an empty mirrored Workspace got stuck in the
“Delete Pending” state.
What’s New in Avid NEXIS 2024.2.0 SSENG-39272 When system drives were rebuilding, the status remained in the Rebuilding state
...
notifications about the upcoming license expiration and some informational license logging messages.
...
- 4000, which was not listed in the Avid Networking Port Usage Guide, and caused problems with
some third-party software. Avid NEXIS clients now use port 7237 instead of port 4000.
...
Network bonding for Linux clients This release includes qualification for Mode 1 network bonding in the Avid NEXIS Client for Linux, providing link redundancy support for Avid FastServe and MediaCentral Cloud UX systems. Mode 1 bonding is also identified as ethTeaming or port-bonding. For more information, see the following documentation:
- FastServe Ingest v2024.3 and FastServe Playout v2023.10 or later: Administrator's Guide for the
relevant FastServe product, available at https://kb.avid.com/pkb/articles/en_ US/Knowledge/FastServe-Documentation.
- MediaCentral Cloud UX v2024.10 or later: Avid MediaCentral | Cloud UX Installation Guide,
available at https://kb.avid.com/pkb/articles/en_US/Knowledge/MediaCentral-CloudUX- Documentation.
Avid NEXIS | Cloud Storage system support for 330 Simultaneous (Active) Clients This release increases the number of clients that can be connected to an Avid NEXIS | Cloud Storage system at the same time, all of which are active. See "Cloud System Limits" on page 21 for more information.
...
USM Versions Change to Client Manager UI for CentOS Due to a code base update in the Avid NEXIS Client Manager, the CentOS kit no longer supports opening the Management Console for a connected Avid NEXIS system. For CentOS clients only, the Client Manager UI no longer includes that button.
Server Fixes SSENG-34814 and SSENG-38562 Avid NEXIS F-Series systems took a long time to report hardware
- failures and other problems.
SSENG-38914 and SSENG-39241 After an upgrade to version 2023.12.0, an Avid NEXIS system sent
- out excessive email notifications about the upcoming license expiration and some informational
license logging messages.
...
When using Microsoft Defender for Endpoint, applications accessing a file on an Avid NEXIS workspace may encounter workflow disruptions if Microsoft Defender for Endpoint also has the same file open. See the Windows-related issue documented in the"Known Issues" on page 25 section.
...
Platforms Current Avid NEXIS Version Can Upgrade Directly To On-premises systems 6.x, 7.x, 2018.x, 2019.x, 2024.11.0 2020.x, 2021.x, 2022.x, 2023.x, 2024.2.x, 2024.5.0, Do not upgrade any on-premises system to 2024.6.x, 2024.10.0 2023.7.0.
...
Avid NEXIS | Cloud 2021.3.1 and newer 2024.11.0 Storage systems To upgrade Avid NEXIS | Cloud Do not upgrade any Avid NEXIS Cloud Storage 2020.7.x and earlier Storage system to 2023.7.0 or 2023.8.0.
...
Avid NEXIS Server and Client Compatibility Avid Avid NEXISAvid NEXIS Avid NEXIS NEXIS Avid NEXIS 2021.x Client Avid NEXIS 2022.x Client 2023.3 2023.8 2023.12 2024.x Client Client Client Client Avid NEXIS 2021 Not recommended for new installations or upgrades N N N server Avid NEXIS 2022 Not recommended for new installations or upgrades N N N server Avid NEXIS 2023.3 Y N N N server Avid NEXIS 2023.8 Not recommended for Not recommended for Y N N N server new installations or new installations or Avid NEXIS 2023.12 upgrades upgrades Y Y Y Y server Avid NEXIS 2024.x Y Y Y Y server Recommended Supported Supported but not recommended for new systems Not supported. Upgrade core (server) to Avid NEXIS 2023.12 or newer ASAP Recommended Version Avid NEXIS 2023.12 is the minimum recommended version that prevents media consistency errors. Newer versions include additional features and fixes. Avid recommends running the latest version of server and client due to the enhanced functionality, security enhancements, and fixes. For a complete list of features and fixes, see the "What's New" section of this ReadMe.
> **Note:** Upgrade Avid NEXIS server to version 2023.12 or higher before upgrading clients.
Avid NEXIS | VFS 2023.7 was withdrawn and not recommended for any system
...
System Director and Client Version Compatibility Operating System Supported Versions Notes
> **Note:** macOS 14.x (Sonoma) with latest updates for Silicon
macOS clients are not M1, M2, M3, and Intel computers with all supported with Avid supported NICs.
NEXIS | Cloud Storage systems.
13.x (Ventura) with latest updates 12.x (Monterey) with latest updates CentOS 7.9, 7.8, 7.7, 7.6 General Avid NEXIS Client use
> **Note:** 7.5, kernel 4.4.174-1.el7 Supported only on
MediaCentral | Cloud UX reached end of life 7.5, kernel 3.10.0-862.el7.x86_64 systems running CentOS 7.5 (EOL) on June 30,
2024. Avid will See the Avid Audio and Video
continue to support Compatibility Charts on the CentOS clients until Avid Knowledge Base page for further notice.
more information on compatibility.
7.4, kernel version 3.10.0-693.17 Specifically for use with AutoDesk Flame 2021; also see "Hardware and Software Requirements" on page 15.
Rocky Linux 9.4 kernel 5.14.0-427.13.1 with 1GbE and Supported only on on-premises 10GbE NICs Avid NEXIS clients Compatible with AutoDesk Flame 2025 Ubuntu 22.04 To migrate MediaCentral | Cloud UX systems from CentOS to Ubuntu, see the MediaCentral Cloud UX Documentation.
See "Ubuntu Client and MediaCentral Cloud UX Compatibility" below Windows 11 build 23H2 10 build 22H2 See "Windows Issues" on page 28.
...
MediaCentral | Cloud UX Server version Avid NEXIS Client version 2023.3 2023.7 2023.12 2024.x 2023.3 Y SLOW PERFORMANCE (~1Gb/s) 2023.8 N Y Y Y 2023.12 N Y Y Y 2024.x N Y Y Y CentOS Version Compatibility with Avid NEXIS Client and VFS Gateway Versions The following versions of CentOS support the specified versions of the Avid NEXIS Client software and the Avid NEXIS | VFS Gateway (formerly Avid NEXIS | Linux File Gateway) software.
On a given CentOS VM version, you can run any of the following versions of the Client and Gateway.
CentOS Version Supported Avid NEXIS Client and VFS Gateway Versions 7.9 2022.9.0 and higher 7.8 2021.3.x through 2022.5.x (inclusive) 7.5 2019.12.0 through 2021.3.x (inclusive)
> **Note:** CentOS Linux 7 reached end of life (EOL) on June 30, 2024. Avid will continue to support CentOS
clients and CentOS VFS Gateway until further notice.
...
Known Issues The following are known issues with Avid NEXIS software or third-party vendors. When a workaround exists, it appears in the paragraph directly following the issue description.
Avid NEXIS On-Premises Systems Issues New SSENG-41851: An Avid NEXIS server and client compatibility issue has been identified that can cause media consistency issues when using Avid NEXIS 2023.8 or a newer client with Avid NEXIS 2023.8 or an older server. To prevent this, Avid recommends using Avid NEXIS v2023.12 or later for both the server and client. For more details, see the "System Director and Client Version Compatibility" on page 15 SSENG-41263: Media Pack log file cannot be downloaded from the Management Console in Avid NEXIS v2024.10. An HTML file with an error is downloaded instead.
...
Known Issues Workaround:You can either ignore this message (it will clear on its own after about 24 hours) or reboot the Engine.
...
Known Issues Mac Platforms and macOS Issues New SSENG-41844: The Avid Benchmark Agent crashes on M-series Apple Silicon clients with Avid NEXIS v2024.11.0.
SSENG-39619: Customized macOS folder icons are not supported in the Avid NEXIS file system.
...
Requirement: On the macOS client, click System Preferences > Security & Privacy > Privacy > Full Disk avidbenchmarkd.
Access and check the box for You might need to provide Administrator credentials to unlock this function (Padlock icon).
...
Windows Issues New SSENG-40046: Avid NEXIS editing clients running on Windows with Microsoft Defender for Endpoint may experience workflow failures if Microsoft Defender for Endpoint holds a file open while the editing application attempts to delete it on drive letter Avid NEXIS mounts. This has prevented proper operation of Send-to-playback workflows.
Workaround: Avid recommends configuring process exclusions in Microsoft Defender for Endpoint for editing applications to avoid workflow interruptions. The Avid Endpoint Security Guidelines provides general information on endpoint security systems. Consult your IT department and refer to the Microsoft Defender for Endpoint documentation for details on configuring process exclusions.
