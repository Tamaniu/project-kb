...
Stack" on page 1, "Creating the Media Pack Instance" on page 17, and and "Creating the Avid NEXIS
...
Recovering a Failed Media Pack By design, an Avid NEXIS | Cloud Storage (AWS Online EBS) system requires four (4) Media Packs. If one fails, you must deploy a new Media Pack stack and add it to the Avid NEXIS system, so that the system can use it to replace the failed one.
