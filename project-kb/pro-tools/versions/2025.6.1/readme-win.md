...
Avid Pro Tools Software v2025.6.1 ReadMe for Windows Tools® This ReadMe documents important compatibility information and known issues for all tiers of Pro
...
Pro Tools Installation PT-260732: AVE remains on a system after performing a full uninstall of Pro Tools and restarting.
Avid Pro Tools Apps & Features > Programs and Workaround: After uninstalling from Windows Features menu and restarting, Avid Video Engine remains installed. To uninstall Avid Video Engine, select it from the same menu and uninstall it as well.
PT-202003: Uninstalling Pro Tools does not uninstall all components that are installed with Pro Tools.
...
PT-258915: With a built-in Soundcard enabled, such as Realtek Audio devices, Pro Tools will not launch.
Assertion in “d:\ code\ protools\ master\ protools\ aae\ source\ aax\ cficaaxwidget.cpp”, line 994.
Workaround: If you are using a built-in Soundcard and Pro Tools will not launch, try the following:
...
General Pro Tools Issues PT-320541: When launching Pro Tools while logged in with a non-administrator account you are prompted for an administrator password.
...
PT-316345: Pro Tools launch stops at “Initializing Audio Engine”.
Workaround: The Pro Tools launch process may stop at “Initializing Audio Engine”. When this occurs, it is necessary to force quit Pro Tools and reboot the computer. This can occur after updating or installing new plugins, or on the first launch of Pro Tools after installation on a new system. This may occur more often on systems with poorly performing hard drives.
...
Workaround: (HDX Systems Only) If you encounter this issue, use scroll wheel on your mouse or trackpad to scroll through the Track List.
...
OK Workaround: If you encounter this issue, toggle the Conductor button off and on. The button should then be usable in the Tempo Change dialog.
...
PT-236243: Subfolders supporting presets menus cannot go deeper than 9 nested folders on windows without issues.
Workaround: Do not create subfolder tree deeper than 9 subfolders for presets. You will not be able to navigate father even if there are other sub-folders (they will be seen only via Explorer).
...
Workaround: With Layered Editing enabled, moving a clip that partially covers a fade on another clip with the Grabber tool so that it no longer covers the fade does not restore the fade to its original duration.
...
or lower results in incorrectly mapped IO.
...
PT-331694: A clip dragged and dropped to an AAX plugin does not link if the session name contains an ellipsis (…).
Workaround: A rendered file created when dragging and dropping a clip to an AAX plugin does not link if there is an ellipsis (…) anywhere in the file path. For AAX drag and drop to function correctly, save the session with a different name and into a different folder that does not contain an ellipsis.
PT-328981: Cannot drag and drop files to Pro Tools when in admin mode.
...
PT-243053: The HD Driver may not load properly after a cold boot when using a HD Native Thunderbolt audio interface with a PCIe Thunderbolt 2 card.
Workaround: Starting up your computer after a full shut down may result in the HD driver not loading properly when using a HD Native Thunderbolt audio interface with PCIe Thunderbolt 2 card. Restart your computer to resolve this issue and the HD driver should load correctly.
...
PT-278737: Improper selection response for Input type and Link controls on S1 surface with Avid PRE Mic Preamp controls.
...
Workaround: When performing a Same as Source bounce to MO, Pro Tools may re-encode Apple ProRes media instead of copying the essence of the original file. If this occurs, the bounce takes longer than expected but should complete without any other problems.
...
PT-244170 and BMD-352: Pro Tools quits unexpectedly on launch if ASIO4All is installed, and a Blackmagic or Avid video peripheral is present and enabled in Windows Device Manager.
Workaround: Pro Tools will not launch if both ASIO4ALL and a Blackmagic video peripheral, or Avid Artist DNxIQ or DNxIO is installed. The launch fails immediately with no error. To avoid this issue, disable the “Avid Artist <device name> AVStream” or “Blackmagic <device name> AVStream” driver found under the “Sound, video and game controllers” heading of the Windows Device Manager. With this disabled, Pro Tools will launch and the video hardware will still function.
...
Pro Tools, or when the video peripheral is disabled. (THPV-32) Video Satellite PT-255060:Playback and locate offsets may be seen when working with a Media Composer Video Satellite that has a 119.88 fps timelineEdoen.
...
PTSW-23142: Pro Tools and Media Composer Video Satellite Quit Unexpectedly when Disconnecting an Additional Pro Tools System During Playback .
...
beds can affect each other, with one of the settings overwriting the other. As a workaround, do not use 5.1 PT-312441: Importing a .pio file created in Pro Tools 2023.9 or earlier results in Dolby Atmos objects being imported into the Bus page of the I/O Setup.
...
dragged and dropped to the timeline. To avoid this, use PT-267165: Object Control Mode must be set to Master in order for Object metadata to be included in an ADM BWF file.
