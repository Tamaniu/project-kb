...
Avid Pro Tools Software v2025.6 ReadMe for macOS Tools® This Read Me documents important compatibility information and known issues for all tiers of Pro
...
Playback Engine dialog PT-267468 32 Sample Hardware Buffer Size is not available.
...
Workaround: If the OK button is not visible in the Preferences window, press Return to save and close the Preferences window (or press Command+. to cancel).
...
PT-207491 Fades are not restored after moving clips using the Grabber tool Layered Editing Workaround: With enabled, moving a clip that partially covers a fade on another clip with the Grabber tool so that it no longer covers the fade does not restore the fade to its original duration.
