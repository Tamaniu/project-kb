...
Avid Media Composer | Distributed Processing Administration Guide Version 2025.12 Using This Guide Using This Guide This document provides instructions for installing, configuring, and using Avid Distributed Processing. Avid recommends that you read all of the information in this document before installing, or using the corresponding software release.
For the latest information on this release, see the following page on the Avid Knowledge Base:
https://kb.avid.com/pkb/articles/en_US/user_guide/Media-Composer-2025-Documentation For additional information on the products and workflows described in this document, see the following:
Avid Media Composer
...
installation processes and basic concepts.
  - Avid MediaCentral | Cloud UX Hardware Guide — Includes information on minimum hardware
specifications as well as scaling and configuration options.
...
Revision History Date Revised Changes Made December 10, 2025 Initial Release Using This Guide Symbols and Conventions Avid documentation uses the following symbols and conventions:
...
1 Distributed Processing Overview 1 Distributed Processing Overview Distributed Processing allows Media Composer users to offload resource intensive and potentially time consuming foreground tasks to one or more service workstations on the local area network. As these tasks are processed outside of Media Composer, users are “unblocked” and have more time to focus on the creative editing process. In this release, you can submit the following jobs to the Distributed Processing system:
- Effect Rendering (including non-Avid effects)
- Transcoding
- Consolidating
- Mixdowns (multiple types)
- Exporting
After you submit a job to Distributed Processing through Media Composer, you can monitor the status of your job through the MediaCentral Cloud UX Process app. As a Media Composer user, you can access this app through the MediaCentral Panel for Media Composer.
If you are a MediaCentral Cloud UX administrator, you have access to the Distributed Processing Workers app. This app allows you to customize the workflow by directing specific job and task types to one or more groups of workers. For more information, see "Using the Distributed Processing Workers App" on page 39.
...
Foyer Service Discovers available media processing operations within all available workers.
1 Distributed Processing Overview Avid Ada Worker and AvidMP Worker Services The Avid Ada Worker Service reads and re-encodes Avid audio media into a format that is consumable by the Avid Media Processor (AvidMP) Worker. This service is installed when you deploy the Avid Ada Feature Pack.
The Avid Media Processor (AvidMP) Worker Service uses the re-encoded media to create a text transcription that can be used with the Avid Ada Transcribe workflow found in Avid MediaCentral Cloud UX v2024.10 and later.
Unlike other Distributed Processing workers, you cannot deploy these services on a Windows or macOS workstation. Avid Ada and AvidMP run on the MediaCentral Cloud UX server(s). If you have installed Distributed Processing for use with Avid Ada Transcribe only, you are not required to deploy any additional workers on Windows or macOS service workstations. The Avid Ada Worker and AvidMP Worker do not participate in any other Distributed Processing tasks, such as transcoding, exporting, or mixing down Avid media.
Also note that neither the Avid Ada Worker service nor the AvidMP Worker appear in the Distributed Processing Workers app.
For more information on installing and configuring these workers, see the Avid MediaCentral | Cloud UX Installation Guide. For more information about the Transcribe workflow that is enabled by the Avid Ada Worker, see "Using the Transcript Tab" in the Avid MediaCentral | Cloud UX User’s Guide.
Client-Based Services Distributed Processing client services are installed on one or more service workstations. For more information, see "Installing a Service Workstation" on page 15.
...
instances of the Media Worker service in a single environment, Avid has qualified support for a maximum of 10 instances in this release of Distributed Processing.
> **Note:** When determining the number of Media Worker services that you want to deploy in your environment,
ensure that you do not exceed the licensing, or bandwidth limits of your Avid NEXIS system 1 Distributed Processing Overview Integrated Systems Distributed Processing workflows require integration with multiple systems. Before you begin, you must verify that you have the required systems and minimum software versions available:
...
- For detailed version information see the Distributed Processing Support Matrix on the Avid Knowledge Base
at: https://kb.avid.com/articles/en_US/compatibility/Avid-Video-Compatibility-Charts 2 Distributed Processing Server Installation 2 Distributed Processing Server Installation This chapter describes the process for installing the Distributed Processing server components on a MediaCentral Cloud UX system. Before you begin the installation process, review the following information.
...
System Security Endpoint detection and response solutions are qualified for use with your MediaCentral Cloud UX system.
While Avid does not support any specific solution, you can find general guidelines and information related to CrowdStrike Falcon on the following Avid Knowledge Base page: https://kb.avid.com/articles/en_ US/troubleshooting/en239659 For additional information related to security for MediaCentral Cloud UX, see “Security Guidelines” in the Avid MediaCentral Cloud UX ReadMe.
Minimum MediaCentral Cloud UX Server Specifications In some cases your organization might already own a MediaCentral Cloud UX server that is integrated with Avid Production Management, or one or more MediaCentral modules. In these cases you must follow the minimum specifications outlined in the Avid MediaCentral | Cloud UX Hardware Guide.
In other cases you might be integrating MediaCentral Cloud UX into your environment to enable Distributed Processing only. When deploying MediaCentral Cloud UX for this single purpose, review the following minimum requirements:
- Memory: 32 GB of RAM
...
The OS volume is used for the operating system and Avid applications. The Data volume hosts the MongoDB database which stores information about user login (avid-iam), license data, and more.
...
Server Virtualization Avid supports installing the Distributed Processing feature pack on a virtualized MediaCentral Cloud UX server, but virtualized service workstations are not supported. For more information, see "Deploying MediaCentral Cloud UX in a Virtual Environment" in the Avid MediaCentral | Cloud UX Installation Guide.
2 Distributed Processing Server Installation MediaCentral Cloud UX Server Installation To install MediaCentral Cloud UX for this release of Distributed Processing, you must have the following:
...
This package includes additional software to install MediaCentral Cloud UX applications (such as the Distributed Processing Workers app) on the Platform.
...
If you plan to deploy a MediaCentral Cloud UX server or cluster with the intention of integrating with Avid Production Management, Asset Management or any of the MediaCentral modules, ignore this abbreviated process and refer to the Avid MediaCentral | Cloud UX Installation Guide for complete instructions.
...
This chapter includes the processes to deploy the operating system and the MediaCentral Platform. It also includes procedures on how to log into the OS for the first time, and how to access the MediaCentral Cloud UX user interface. Complete the following sections of this chapter:
  ...
  - (if applicable) Configuring Avid Ada Nodes (required for Avid Ada Transcribe)
  - Creating a Site Key
  - Creating Certificate Files
  - Deploying the TLS Certificates
2 Distributed Processing Server Installation
  - Configuring an Authentication Provider
  - (if applicable) Configuring the Avid XFER and XFORM Services (required for Avid Ada
Transcribe)
  - (if applicable) Configuring the Change Event Agent (required for Avid Ada Transcribe)
  - Enabling System Monitoring
  - (if applicable) Configuring Audit Logging (optional for Avid Ada Transcribe)
  - Running the Feature Pack Deployment Script
Yes When deploying the feature packs, you must answer to the Distributed Processing and final “Feature packs” (additional feature packs common to all installations) prompts and answer No for all other feature packs.
> **Note:** In the event that you are deploying both Distributed Processing and Media
...
Before you can use Distributed Processing, you must license the server for the feature. The following illustration shows the two licenses associated with Distributed Processing.
Distributed Processing Worker This license governs the number of workers that can process media at the same time. Although the worker checks for a license when the service is launched, you can install more instances of the Worker Service than are allowed by your Worker license. The license count restricts the number of simultaneously running tasks, but not the workers themselves. If more workers are running than the license count allows, any worker which meets the filtering criteria for a specific task can accept the task.
For more information, see "Installing a Service Workstation" on page 15.
...
For more details on applying licenses, see the Avid MediaCentral | Cloud UX Installation Guide.
Distributed Processing Engine This license enables the Distributed Processing feature on the server.
2 Distributed Processing Server Installation Configuring Licenses Exclusively for Distributed Processing If you deploy a MediaCentral Cloud UX system for a Distributed Processing-only workflow, you must ensure that your server is configured for the following licenses to enable integration between the systems:
...
MediaCentral Cloud UX User Entitlements The MediaCentral Cloud UX User Management app allows administrators to import users and configure the entitlements that are associated with each user group. In the context of Distributed Processing, the Master of Jobs entitlement is the only applicable entitlement.
Master of Jobs When signed into MediaCentral Cloud UX as a standard user, the Process app displays only those jobs that you have personally submitted. Users with the Master of Jobs entitlement can see the list of jobs submitted by all users.
For more information, see “Using the User Management App” in the Avid MediaCentral | Cloud UX Installation Guide.
Working with the Kubernetes Dashboard The Linux OS on your Distributed Processing server is built in a Kubernetes® managed container structure.
...
avid-ui-distributed-processing-workers
...
a pod, you must refresh the MediaCentral Cloud UX user interface after Kubernetes reports that the pod has been successfully recreated.
If you do not see the Distributed Processing Workers app in the user interface after installing and licensing the application, you can attempt to delete the avid-plugins-list-core-app pod.
For more information, see the Avid MediaCentral | Cloud UX Installation Guide.
3 Installing a Service Workstation 3 Installing a Service Workstation When you enable a Distributed Processing workflow, you must install the client services on one or more Microsoft Windows or macOS systems. These services might be installed on server-class hardware or on client workstations that meet the minimum specifications outlined in this chapter. Throughout this document, the term service workstation is used to refer to any server or workstation that hosts the client- based Distributed Processing services.
...
For a list of supported operating systems, see the Distributed Processing Support Matrix on the Avid Knowledge Base at:
...
If you are deploying Distributed Processing for use with Avid MediaCentral | Shared Library only, the requirements for that configuration are reduced. For more information, see v2019.9 or later of the Avid MediaCentral | Shared Library Installation Guide.
Minimum Specifications If you intend to install one or more Distributed Processing services on a single service workstation, you must meet the following minimum specifications.
...
3 Installing a Service Workstation High-End Specifications If Distributed Processing is under heavy load in your environment, you might consider purchasing a workstation or server with more robust system resources. The following table outlines the specifications for a higher-end service workstation. Avid recommends that you meet or exceed these guidelines.
...
System Security
  - This version of Distributed Processing is qualified with Microsoft Defender Firewall and
Microsoft Defender Antivirus enabled on dedicated service workstations. These same qualifications might or might not apply to Distributed Processing services that are co-installed on a Media Composer client. Refer to the Media Composer documentation for Media Composer-specific and version-specific guidelines.
...
the specific job or task type enabled. For more information on queues and groups, see "Using the Distributed Processing Workers App" on page 39.
...
Workstations using either UNC paths or drive letters.
...
Installing Distributed Processing for Windows Complete the following process to install the Distributed Processing software on one or more service workstations running Microsoft Windows.
...
5. Click the Next button in the Welcome to the InstallShield Wizard for Distributed Processing screen.
...
If you are not prepared to install the Distributed Processing software at this time, you can click the Cancel button to exit the installation.
...
9. Proceed to "Using the Distributed Processing Worker Settings App" on page 21 to enter some required
the configuration settings.
Installing Distributed Processing for macOS Complete the following process to install the Distributed Processing software on one or more service workstations running macOS.
To install the Distributed Processing software:
...
4. In the Distributed Processing Install Introduction screen, click the Continue button to begin the
...
You can obtain version information about the app by right-clicking it in the Windows System Tray or the macOS Menu Bar and select About.
To configure or reconfigure the services:
1. Right-click the Distributed Processing Worker Settings icon in the Windows System Tray or the
macOS Menu Bar and select Open.
...
a. Avid MediaCentral | Cloud UX Enter the IP address, or fully qualified domain name of your Avid MediaCentral Cloud UX server or cluster.
...
Processing services might vary depending on your project format, media, effects, and other factors. If you install multiple workers and discover a noticeable decrease in system performance, you might need to alter your configuration to decrease the number 3 Installing a Service Workstation of instances on the Media Worker Service.
  - Licenses: If you increase the number of workers, you must remember that the Worker
license count determines the maximum number of simultaneous tasks that can be processed by the system. See "Distributed Processing Licensing" on page 13 for a review of your license options.
...
d. Allow Worker to Run If you are performing a co-installation on a Media Composer client, note the following:
...
e. Tasks to Process This menu allows you to select one of two options:
...
If you select the “From Local” option, you can still prevent your local workstation from processing tasks created by the co-installed instance of Media Composer by altering the Media Composer Media Creation Settings. For more information, see "Adjusting the Media Creation Settings" on page 28.
If Media Composer is not running (launched), the Worker Service ignores this setting.
3. Click the Start button to start the Distributed Processing services on your workstation.
If the app cannot start the services for any reason, you can review the DistributedProcessingSettings.log file for details. For more information on system log files, see "Distributed Processing Logs" on page 24.
4. Click the X in the upper-right corner of the app to exit the user interface.
You can reopen the settings at any time by double-clicking the app icon in the Windows System Tray or the macOS Menu Bar.
After starting the services, the app displays a “Running” status. However, the services might take a few moments beyond this status update to fully come up to speed. If the service workstation fails to connect to the Distributed Processing server, it retries up to 10 times. If the connection fails, the settings app stops the local services.
...
3 Installing a Service Workstation Quitting the Settings App The Distributed Processing Worker Settings app allows you to either close or quit the application. If you close it, the application remains active — represented by the icon in the Windows System Tray or macOS Menu Bar. If you select the quit option, the application shuts down completely.
To quit the Settings app and stop the services:
- Right-click the Distributed Processes Settings icon in the Windows System Tray or the macOS Menu
...
3 Installing a Service Workstation Distributed Processing Logs Each of the Distributed Processing services creates log files that can be referenced for troubleshooting purposes.
...
Features.
4. Navigate to the Distributed Processing in the list of Apps and Features.
5. Select Distributed Processing.
6. Click Uninstall.
...
Media Composer Prerequisites Review the following information that relates to Distributed Processing workflows.
...
> **Note:** Avid does not support Distributed Processing with Media Composer | First or with Media Composer |
Subscription licenses. Media Composer | Ultimate and Media Composer | Enterprise subscriptions are supported.
...
Linked Assets You can Consolidate orTranscode media located on non-Avid NEXIS, network attached storage to an Avid NEXIS system. However this workflow is supported on Windows only.
...
User Accounts You must sign into the Media Composer Project window using your MediaCentral Cloud UX user credentials. Your server must be integrated with Windows Active Directory and all Media Composer users must be included in User Groups that have a valid Client License (Browse, Edit) assigned to them. For more information about these processes, see the Avid MediaCentral | Cloud UX Installation Guide.
> **Note:** Distributed Processing does not currently support an integration with Open ID authentication
providers.
...
Connecting to the Distributed Processing Server Media Composer users must connect to the MediaCentral Platform to enable the Distributed Processing workflow. This is accomplished by signing into the MediaCentral Cloud UX system in the Media Composer Select Project window. The following illustration shows the Project window with the Login section highlighted.
...
MediaCentral Cloud UX cluster, enter the cluster’s virtual host name.
...
4. Click the Login button to connect to your MediaCentral Cloud UX system, and then click OK to
access your project.
...
By default, temporary files are created in the DistributedProcessing folder in the root-level of the target Avid NEXIS workspace that is selected for the job. The system monitors these temporary files and automatically deletes them after three days.
...
Enable this option if you want to open the MediaCentral Panel for Media Composer automatically after you submit a job.
...
group to which your local client is allocated in the Distributed Processing Workers app.
For more information, see "The Distributed Processing Workers App" on page 39.
- Automatically Check in Sequence Sent for Render
If you are working in an environment that includes Production Management, you can select this option to automatically check your sequence into the Production Management database following a successful render job.
...
4 Distributed Processing Workflow Rendering Effects with Distributed Processing Users can submit Render jobs to Distributed Processing using the following process.
To render effects using Distributed Processing:
...
If the window includes a Create Proxy option, that feature is not applicable to this version of Distributed Processing.
...
8. (optional) Check the status of your job in the MediaCentral Cloud UX Process app. For additional
details, see "Viewing the Job Status" on page 37.
Consolidating or Transcoding Assets Users can submit Consolidate or Transcode jobs to Distributed Processing using the following process.
For more details on other options in the Consolidate/Transcode menu refer to the Avid Media Composer Editing Guide.
To consolidate or transcode assets using Distributed Processing:
...
4 Distributed Processing Workflow Avid does not recommend enabling the Create New Sequence or Create New Clip options in this window. The Distributed Processing workflow always creates a new sequence or clip, so enabling this option creates a redundant copy of the asset metadata.
...
9. (optional) Check the status of your job in the MediaCentral Cloud UX Process app. For additional
details, see "Viewing the Job Status" on page 37.
...
The mixdown master clip appears in the same bin as your sequence, however the associated media is only available after the Distributed Processing job is complete.
5. (optional) Check the status of your job in the MediaCentral Cloud UX Process app. For additional
details, see "Viewing the Job Status" on page 37.
...
4 Distributed Processing Workflow To export an asset using Distributed Processing:
...
4 Distributed Processing Workflow This menu defines the group of workers that will process your job’s tasks. If for example your administrator created an “Export Only” queue, you might need to select that option for your export job.
If Media Composer has access to only a single queue, the Job Queue menu is not displayed.
...
If you enabled the Pre-Processing option and your sequence includes any unrendered effects, the system renders all the required effects before the export task is initiated.
8. (optional) Check the status of your job in the MediaCentral Cloud UX Process app. For additional
details, see "Viewing the Job Status" below.
...
Viewing the Job Status After you submit a Distributed Processing job, you can watch the process of the job through the Process app. You can access the Process app through MediaCentral Panel for Media Composer.
When thinking about the Distributed Processing workflow, you should note the difference between jobs and tasks. As a user, you can submit render, export, or other jobs to the Distributed Processing system. You can then track the progress of your job through the MediaCentral Cloud UX Process app.
Distributed Processing associates every job with at least one task. Depending on the job type and complexity, the system might subdivide the job into multiple tasks that can be distributed across two or more workers for faster processing. As shown in the following illustration, the Process app allows you to expand the job so that you can monitor the progress of each individual task.
4 Distributed Processing Workflow
> **Note:** Distributed Processing metadata tasks do not provide any status update and remain in a permanent
"Created" state.
...
If your job results in a failure, you can use the Process app to obtain more specific information about the job.
For more information about viewing the status of Distributed Processing jobs, see "Working with the Process App" in the Avid MediaCentral | Cloud UX User's Guide.
5 The Distributed Processing Workers App 5 The Distributed Processing Workers App The Distributed Processing Workers app allows you to monitor and configure the workers that are connected to the system. You can group workers by similar tasks, resources, or user needs.
Accessing the Distributed Processing Workers App You can access the Status app through MediaCentral Cloud UX using a web browser.
To access the app through a web browser:
1. Log in to the Avid MediaCentral Cloud UX administrator's portal through a web browser.
For more information, see “Signing In to MediaCentral Cloud UX” in the Avid MediaCentral | Cloud UX Installation Guide.
2. Click on the Distributed Processing Workers app in the Fast Bar.
Using the Distributed Processing Workers App The Distributed Processing Workers app allows you to create groups of workers and define the types of jobs and tasks that will be processed by those workers. With this level of control, you can customize your Distributed Processing system to better accommodate to your site’s workflows and available resources. For example, you might consider creating “high priority” group that includes only the service workstations with the most powerful system resources within your organization. Alternatively, you could create multiple groups that are dedicated to certain tasks, such as exporting only, or effect rendering only.
As illustrated below, the app is organized into three virtual columns of data: Job Queues, Worker Groups, and Workers. Each block or card represents an individual queue, group, or worker.
5 The Distributed Processing Workers App Notice that in this example, the app shows three worker cards. However, two of these worker cards show the same host name “z820”. This indicates that the Distributed Processing system includes two services workstations (z820 and DESKTOP-98RE), one of which (z820) is running two instances of the Worker Service.
...
- For more information, see "Managing Worker Groups" on page 45.
Workers: Perform media processing tasks for the jobs submitted to the selected queue.
- For more information, see "Managing Workers" on page 47.
Every Distributed Processing installation includes a Default Job Queue and a Default Worker Group.
Whenever you add a worker to the system, the worker is added to the default queue and group automatically (on first launch). While you can edit the description of the default queue and group, you cannot edit their names or delete them.
...
Refreshing the Display If you make a change in the app and you do not yet see the change displayed, you can click the Refresh button in the Coordinator Settings panel to update the display.
5 The Distributed Processing Workers App Working with Job Queues Queues are the entry points for submitting any Distributed Proceeding operation. They provide you with global control over the types of jobs that can be processed by the associated workers. When you create a queue, all workflows are enabled by default. If you leave them enabled, you can use Groups to manage the types of tasks that can be processed by the workers. Consider the following illustrations and examples.
Example 1: Separate Workers by Queue/Group You can create additional queues (and groups) to separate the workers to avoid job processing overlap. In the following illustration, the administrator created one new queue (and group) and divided the workers between the default queue and the new queue. In this case, all job types are enabled for both groups of workers.
Example 2: Manage by Queue You can disable certain job types for individual queues. In the following example, the administrator disabled mixdown, render, and export on Editors Group 2. If any of these job types are enabled in Worker Group A, the workers would not process any of these tasks because the associated queue has those features disabled.
If a user submits a render job to Editors Group 2, the job is displayed in the Process App. However, the Process app will display a status of Pending until you either cancel the job, or reconfigure a queue to allow the job type.
5 The Distributed Processing Workers App You are not required to disable job type per queue, but this practice can help you isolate associated workers from running particular types of jobs for better resource management.
Example 3: Manage by Group You can associate multiple groups with a single queue to manage workers by group. In this example the administrator created and connected Worker Group A and Worker Group B to the Editors Group 2 queue.
Within a single queue, you could create one worker group that allows transcode and consolidate operations and associate it with workers that do not have GPU or third-party plugins installed. This group of workers would be dedicated to transcode and consolidate operations and would be excluded from "heavier" tasks such as render or export.
5 The Distributed Processing Workers App You could create another worker group and assign high-performance workstations to it. There might be multiple options how to configure it:
You could limit this group to support render and export operations only. As long as those job types
- are disabled in the other worker group, you could submit any type of job to the queue, and all render
and export tasks would be directed to those workers in Worker Group B.
- Alternatively, you could allow all types of operations for this worker group. Assuming that Worker
Group A has render and export disabled, all workers assigned to Worker Group B can process all job types, but only the workers in Worker Group B can process render and export jobs.
This section includes the following topics:
- "Creating a Job Queue" on the next page
- "Configuring Job Queues" on the next page
5 The Distributed Processing Workers App Creating a Job Queue You are required to create additional queues only if you want to customize your workflow beyond the default queue. If a single queue is appropriate for your workflow, you can review the process for "Configuring Job Queues" below to customize the default queue.
...
1. Select the queue in the Coordinator Settings panel.
The app displays the queue parameters in the Details panel to the right.
...
Avid suggests that you keep the name brief so that the user interface of other applications, such as Media Composer, do not truncate the name.
  - Description: You can enter text in this field to explain the purpose of the queue.
...
- Click the Cancel button to revert your changes.
5 The Distributed Processing Workers App Deleting Queues You can use the following process to delete a custom queue. The app does not allow you to delete the default job queue.
To delete a queue:
1. Hover your cursor over the queue and click the Delete button that appears to the right of the name.
2. Click the Delete button to remove the queue from the app.
The queue is deleted. If you had any worker groups associated with this queue, they are outlined in a white border to note that are no longer associated with any queue.
Managing Worker Groups Worker Groups allows you to group workers by any parameter. This might mean that workers are installed on similar workstations, have similar plugins installed, or are designated to perform specific media processing operations. Distributed Processing allows you to create as many groups as your workflow requires. However, each group must be associated with one queue and at least one worker before it becomes active. You cannot associate a single group with more than one queue.
This section includes the following topics:
- "Creating a Worker Group" below
- "Configuring a Worker Group" below
- "Managing Worker Groups" above
...
A new group appears with a default name of "Worker Group" is added to the bottom of the Worker Groups column. If you do not see your new group, you can use the scroll bar on the right side of the user interface to navigate to it.
...
5 The Distributed Processing Workers App To edit or configure the group’s parameters:
1. Select the group in the Coordinator Settings panel.
The app displays the group’s parameters.
2. Configure or edit the following values:
  - Name: Click inside the text field to enter a new name for the group.
  - Description: You can enter text in this field to explain the purpose of the group.
...
- Click the Save button to confirm your changes.
- Click the Cancel button to revert your changes.
Deleting a Worker Group Complete the following process to delete a group from the system. The app does not allow you to delete the default worker group.
...
To delete a group:
1. Hover your cursor over the group card and click the Delete button that appears to the right of the
name.
The system displays a confirmation window.
2. Click the Delete button to remove the group from the app.
The group is deleted.
5 The Distributed Processing Workers App Managing Workers If you select a worker in the Coordinator Settings panel the right-side panel displays additional information about the worker. If your workflow includes multiple groups, you can use this information to help you make more informed decisions about how to organize your workers. The following illustration and table provide an explanation about the different sections of this panel.
# Name Description Worker Displays the worker hostname, the status of the worker (Ready or Working), and the type of the task that the worker is currently executing. If the worker is not actively processing any jobs, the Task field is left blank.
The following illustration shows the status and task fields. Note that the task is also reflected on the worker card.
5 The Distributed Processing Workers App # Name Description The Assign to Worker Group menu displays the current group in which this worker is currently assigned. You can also use this menu to assign the worker to a different group.
2 Versions This section displays the version of the Distributed Processing software and Avid NEXIS Client that is installed on the service workstation.
The Worker API connects to the Distributed Processing Engine on MediaCentral Cloud UX. This value is displayed as orange text if Worker is running a version of software that is incompatible. with the Engine. In this case the worker remains active, but it does not participate in the workflow.
3 System Displays additional information about the service workstation, including the version of the operating system, installed memory, and more.
4 Plugins This section lists the AMA, AVX2, and Export plug-ins that are installed on this service workstation.
5 Mounted This section displays the Avid NEXIS workspaces that are mounted on the service Workspaces workstation, along with the drive letter mount points.
6 Settings The Tasks to Process relates to the setting of the same name that you configured in the Settings. For more details on the Any or "From Local Media Composer" options, see "Using the Distributed Processing Worker Settings App" on page 21.
As you know, some service workstations might run one or more instances of the Avid Worker Service. The cards that are displayed in the Coordinator Settings panel are a visual representation of each instance of the Worker Service service.
...
Since all workers have the ability to process all task types, you can move workers without worrying about their local system resources. The one exception to this rule is a service workstation’s ability to render 3rd party effects. If you require a worker to process a particular effect, the effect software must be installed and licensed on that worker. When you submit a job that includes a 3rd party effect, Distributed Processing automatically directs the render task to the worker that includes the effect software. Therefore, you are not required to install the 3rd party software on all workers.
5 The Distributed Processing Workers App To assign a worker to a group:
- Select the worker and use the "Assign to Group" menu in the Details panel to change the group
association.
- Drag and drop the card to the desired group.
The following illustration shows an administrator dragging one of the workers in the default group to Worker Group B.
- Lasso or Ctrl+select multiple workers in the Coordinator Settings panel and use the Batch Assign to
Group menu in the Details panel to assign multiple workers to a new group simultaneously.
...
- appear in the Distributed Processing Workers app.
- Uninstall the Distributed Processing services from the worker.
For more information, see "Uninstalling the Distributed Processing Software" on page 25.